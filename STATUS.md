...
| Llama 3 8B Instruct Q8_0 | The 8B row is no longer groundwork-only: compact parity, broader 50-token parity, API/WebUI, 512-context, compact template-shapes, memory evidence, and hot-path measurements are public. | 512 bounded pack is green; 1024 and 2048 are not promoted. | Exact blocker: llama.cpp accepted the 1024/2048 prompts, but Camelid `/v1/chat/completions` did not complete within the 900-second parity harness timeout, so no PASS parity reports exist. |
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked 512-pack result only, not a 1k/2k/model-native context or portability claim. The exact Llama 3.2 1B and 3B rows also have separate checked 1024-context pack passes, both exact Llama 3.2 rows have separate checked 2048-context pack passes, and the exact 8B 1024/2048-context attempts remain red on backend timeout; each remains limited to its row and prompt pack.
