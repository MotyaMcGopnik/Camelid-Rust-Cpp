...
Recent Mixtral hardening note: continuation/long-generation work is active. The current hardening slice adds exact prompt-token continuation input for reproducible follow-on generation and keeps the checked short-prompt support claim unchanged while the longer-generation lane is still partial.
...
- `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-longgen-continuation-20260511/` is partial hardening evidence only: one 128-token `Hello` run completed without parity, and a later `What is 2+2?` backend call hung after the llama.cpp reference finished. This is a debugging lane, not a support expansion artifact.
