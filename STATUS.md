...
| Llama 3 8B Instruct Q8_0 | The 8B row is no longer groundwork-only: compact parity, broader 50-token parity, API/WebUI, 512-context, compact template-shapes, memory evidence, and hot-path measurements are public. | 512 bounded pack is green for this exact row; 1024/2048 remain not promoted. | Hard boundary: 1024/2048 need fresh PASS artifacts plus docs/API/frontend alignment before support wording moves; no model-native/larger context, broad 8B/Llama support, arbitrary templates, production throughput, or portability. |
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end short-context generation parity artifacts: compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and the checked 512-context pack all agree for the exact tracked Q8_0 GGUF. The 8B 1024 and 2048 bounded context boxes remain not promoted until fresh PASS artifacts and docs/API/frontend alignment land together.
...
- `qa/validation-notes/2026-05-05-llama3-8b-context-1024-blocker.md` records the earlier required 1024 diagnostic timeout for the exact Llama 3 8B Instruct Q8_0 row; keep it as red-box history for the still-unpromoted 1024 bucket.
- `qa/validation-notes/2026-05-05-llama3-8b-context-2048-blocker.md` records the earlier failed 2048 attempt; keep it as red-box history for the still-unpromoted 2048 bucket.
- `qa/evidence-bundles/llama3-8b-context-1024-20260506T182100Z-head-e146d3b335d8/manifest.json` and `qa/evidence-bundles/llama3-8b-context-2048-20260506T182534Z-head-e146d3b335d8/manifest.json` remain review artifacts only in this support surface; they do not promote 8B 1024/2048 without fresh PASS artifact review and synchronized docs/API/frontend changes.
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows only; the Llama 3 8B row reports those buckets as not promoted, and frontend surfaces display that boundary.
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked 512-pack result only, not a 1k/2k/model-native context or portability claim. The exact Llama 3.2 1B and 3B rows also have separate checked 1024-context pack passes, and both exact Llama 3.2 rows have separate checked 2048-context pack passes. The exact 8B 1024/2048 buckets remain not promoted until fresh PASS artifacts and docs/API/frontend alignment land together.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. Later 8B 1024/2048 bounded-pack support still needs fresh PASS artifacts and synchronized docs/API/frontend promotion.
...
- The 1024/2048 artifacts are not current support truth for this exact 8B row. Keep them in review history until a fresh PASS artifact set and synchronized docs/API/frontend promotion land together.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through checked 512-context; neighboring Llama rows, other quantizations, 1024/2048, larger/model-native contexts, and broader chat-template behavior remain outside the claim.
