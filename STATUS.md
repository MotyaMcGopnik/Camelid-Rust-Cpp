...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved, the 8B broader three-prompt 50-token rerun passed, and the exact Llama 3.2 1B/3B/8B rows have bounded 512/1024/2048-context packs green where row-specific PASS artifacts exist. The current-head 8B 1024/2048 bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T062823Z-head-86081876d01e/manifest.json` passed at source head `86081876d01e` with prompt-token, generated-token, generated-text, RSS, and Q8 file-read counters; older 8B 1024/2048 bundles remain historical for their source heads. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity envelopes; broad Llama-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, production throughput, and portability remain outside the support claim.
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, the checked 512-context pack, and the current-head checked 1024/2048-context packs for the exact tracked Q8_0 GGUF. The current 1024/2048 bounded prompt-pack pass is `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T062823Z-head-86081876d01e/manifest.json`; the earlier `e203d3cf3ea5` pass is historical for that source head.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T062823Z-head-86081876d01e/manifest.json` plus `SHA256SUMS` is current-head 8B 1024/2048 bounded-pack PASS evidence for source head `86081876d01e`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`, and includes structured RSS/Q8 file-read counters. It promotes only those exact bounded packs for the exact 8B Q8_0 row.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B/8B rows where row-specific PASS artifacts exist.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. The current 8B 1024/2048 promotion depends on the separate `86081876d01e` PASS bundle with row-specific parity, RSS, and Q8 file-read evidence.
...
- The current 8B context support truth for this exact row includes the checked 512/1024/2048 bounded packs. The 1024/2048 pass at `86081876d01e` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T062823Z-head-86081876d01e/manifest.json`; the earlier `e203d3cf3ea5` bundle remains historical source-head evidence.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T062823Z-head-86081876d01e/manifest.json`
...
- the current-head 1024/2048 bounded-context pack passed with prompt-token, generated-token, and generated-text parity plus structured RSS/Q8 file-read counters; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T062823Z-head-86081876d01e/`
...
This is a support promotion only for the exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope, including the checked bounded 512/1024/2048 context packs. It is not broad Llama-family, model-native/larger-context, production-throughput, portability, arbitrary-template, or neighboring-row support.
