...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved and the 8B broader three-prompt 50-token rerun passed. Llama 3.2 1B/3B/8B retain bounded 512/1024/2048-context packs where row-specific PASS bundles are cited. The fresh 8B 1024/2048 bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T173841Z-head-aa315788ce2e/manifest.json` passed at runtime/API/frontend head `aa315788ce2e` with prompt-token, generated-token, generated-text, RSS, and Q8 file-read counters. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity envelopes; broad Llama-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, production throughput, and portability remain outside the support claim.
...
| Llama 3 8B Instruct Q8_0 | The 8B row is no longer groundwork-only: compact parity, broader 50-token parity, API/WebUI, the checked 512/1024/2048-context packs, compact template-shapes, memory evidence, structured Q8 read counters, and hot-path measurements are public. | 512/1024/2048 bounded packs are green for this exact row where cited; the fresh 1024/2048 bundle covers runtime/API/frontend head `aa315788ce2e`. | Hard boundary: no model-native/larger context beyond checked packs, broad 8B/Llama support, arbitrary templates, production throughput, portability, or neighboring-row/context promotion without row-specific evidence. |

This is the repo story in one sentence: Camelid expanded from one trusted small-model gate to a four-row bounded Llama/TinyLlama contract plus one exact Mistral first-token smoke lane, while preserving fail-closed support language for every unproven neighboring row, context, quantization, and runtime path.
...
- **Next-model bring-up set:** Camelid has moved the first Mistral exact row to first-token smoke support while keeping the other next-family rows planned.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — exact-row first-token smoke: tokenizer/template references and 1-token generated parity are closed; broader/full support remains blocked on 50-token, API/WebUI, RSS/timing, 1024/2048, throughput, portability, and repeated current-head evidence.
...
Public readiness language is locked to exact rows: Mistral may say “exact-row first-token smoke support only,” while Mixtral, Qwen, and Gemma may say “planned exact-row candidate, not supported yet.” Broader promotion requires row-specific source/SHA/license, tokenizer/template references, bounded load/readiness, parity, API/WebUI, RSS/timing, scrubbed manifest, and checksum evidence; Mixtral also needs expert-routing proof or typed unsupported behavior.
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and the checked 512/1024/2048-context packs for the exact tracked Q8_0 GGUF. The fresh 1024/2048 bounded prompt-pack pass at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T173841Z-head-aa315788ce2e/manifest.json` covers runtime/API/frontend head `aa315788ce2e`; earlier `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` passes remain historical source-head evidence.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T173841Z-head-aa315788ce2e/manifest.json` plus `SHA256SUMS` is the fresh exact Llama 3 8B Q8_0 bounded 1024/2048 PASS bundle for runtime/API/frontend head `aa315788ce2e`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`, and includes structured RSS/Q8 file-read counters. It promotes only those exact bounded packs, not model-native/larger context, arbitrary templates, neighboring rows, broad 8B/Llama-family support, portability, or production throughput.
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T122749Z-head-9f8588bb4a4e/manifest.json` plus `SHA256SUMS` is historical source-head 8B 1024/2048 bounded-pack PASS evidence for source head `9f8588bb4a4e`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`, and includes structured RSS/Q8 file-read counters.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B/8B rows where row-specific PASS artifacts exist; the 8B 1024/2048 promotion is limited to the fresh bounded-pack evidence for runtime/API/frontend head `aa315788ce2e`.
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked bounded-pack result only, not a model-native context or portability claim. The exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B rows also have checked support through 1024-context and 2048-context packs where listed.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. The fresh `aa315788ce2e` 8B 1024/2048 PASS bundle is the row-specific bounded-pack evidence for that exact 8B promotion.
...
- The current 8B context support truth for this exact row includes the checked 512/1024/2048 bounded packs where cited. The fresh 1024/2048 pass for runtime/API/frontend head `aa315788ce2e` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T173841Z-head-aa315788ce2e/manifest.json`; the earlier `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` bundles remain historical source-head evidence.
...
- the 1024/2048 bounded-context pack passed at runtime/API/frontend head `aa315788ce2e` with prompt-token, generated-token, and generated-text parity plus structured RSS/Q8 file-read counters; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T173841Z-head-aa315788ce2e/`; older 1024/2048 bundles remain historical for their source heads
...
6. Continue the first Mistral exact-row closure lane beyond first-token smoke: the chosen acceptance target is `Mistral-7B-Instruct-v0.3.Q8_0.gguf`; tokenizer/template parity and 1-token generation parity are closed, but broad/full Mistral stays blocked until 50-token output, API/WebUI smoke, RSS/timing, 1024/2048 context, throughput, portability, and repeated current-head evidence exist.
...
Mistral work has its first narrow green box. Camelid has locked `Mistral-7B-Instruct-v0.3.Q8_0.gguf` as the immediate closure target and now has checked tokenizer/chat-template prompt references plus 1-token generated-token/text parity (`[23325]` / ` Hello`) in `qa/evidence-bundles/mistral-7b-v0.3-q8-1tok-parity-20260508T1847Z-head-fa7efc086c0e/`. Public docs may call this exact-row first-token smoke support only. Phrases like “Mistral-family support,” “full Mistral support,” or neighboring-row support remain premature until row-specific broader prompt/output, API/WebUI, context, perf, portability, and repeated current-head smoke artifacts exist.

The next Mistral evidence bundle should add broader prompt packs, 50-token generation parity, API smoke, frontend smoke, bounded 1024/2048 contexts, RSS/timing notes, scrubbed manifests, and checksums. Promote those boxes one at a time.
