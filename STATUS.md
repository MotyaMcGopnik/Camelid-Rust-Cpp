...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved and the 8B broader three-prompt 50-token rerun passed. On current `main`, Llama 3.2 1B/3B retain bounded 512/1024/2048-context packs, while Llama 3 8B is promoted only through the checked bounded 512-context pack. The 8B 1024/2048 bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/manifest.json` passed at source head `844e8d4709d1` with prompt-token, generated-token, generated-text, RSS, and Q8 file-read counters, but later source/runtime changes make it historical until a fresh current-head canonical PASS lands. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity envelopes; broad Llama-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, production throughput, and portability remain outside the support claim.
...
- **Active next-model bring-up set:** Camelid is now publicly working on four exact next-family rows, with one immediate closure lane and three follow-on candidates.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — immediate closure lane; Ubuntu load/serve evidence exists, but tokenizer/template parity remains the blocker.
...
Public readiness language for these four rows is locked to family-by-family exact-row planning: Mistral may say “in active validation, not supported yet,” while Mixtral, Qwen, and Gemma may say “planned exact-row candidate, not supported yet.” First promotion requires row-specific source/SHA/license, tokenizer/template references, bounded load/readiness, parity, API/WebUI, RSS/timing, scrubbed manifest, and checksum evidence; Mixtral also needs expert-routing proof or typed unsupported behavior.
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and the checked 512-context pack for the exact tracked Q8_0 GGUF. The 1024/2048 bounded prompt-pack pass at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/manifest.json` is source-head evidence for `844e8d4709d1`; after later source/runtime changes it is historical, along with the earlier `86081876d01e` and `e203d3cf3ea5` passes, until a fresh current-head PASS lands.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/manifest.json` plus `SHA256SUMS` is source-head 8B 1024/2048 bounded-pack PASS evidence for source head `844e8d4709d1`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`, and includes structured RSS/Q8 file-read counters. After later source/runtime changes, it is historical evidence only and does not promote 8B 1024/2048 on current `main` without a fresh canonical PASS.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows with row-specific PASS artifacts, while keeping 8B 1024/2048 unpromoted until fresh current-head artifacts and docs/API/frontend alignment exist.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. The `844e8d4709d1` 8B 1024/2048 PASS bundle remains source-head evidence and does not cover later runtime/source changes.
...
- The current 8B context support truth for this exact row includes the checked 512 bounded pack only. The 1024/2048 pass at `844e8d4709d1` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/manifest.json`, but it is source-head evidence after later runtime/source changes; the earlier `86081876d01e` and `e203d3cf3ea5` bundles remain historical source-head evidence too.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through the checked 512 bounded context on current `main`; 1024/2048 need a fresh current-head PASS before promotion. Neighboring Llama rows, other quantizations, larger/model-native contexts beyond checked packs, production throughput, portability, arbitrary templates, and broader chat-template behavior remain outside the claim until their own evidence exists.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/manifest.json`
...
- the 1024/2048 bounded-context pack passed at source head `844e8d4709d1` with prompt-token, generated-token, and generated-text parity plus structured RSS/Q8 file-read counters; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/`, but later source/runtime changes make it historical until rerun
...
This is a support promotion only for the exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope, including the checked bounded 512 context pack on current `main`; 1024/2048 remain unpromoted until a fresh current-head PASS lands. It is not broad Llama-family, model-native/larger-context, production-throughput, portability, arbitrary-template, or neighboring-row support.
...
6. Drive the first Mistral exact-row closure lane as planned work only: the chosen acceptance target is `Mistral-7B-Instruct-v0.3.Q8_0.gguf`; preserve the Ubuntu load/serve evidence, close tokenizer/template parity with known-good prompt-token references, preserve fail-closed readiness behavior, and do not expose Mistral as supported until bounded row-specific artifacts exist.
...
Mistral work is now open as the first exact-row bring-up track, but it has no validated release evidence and no runtime support claim yet. Camelid has locked `Mistral-7B-Instruct-v0.3.Q8_0.gguf` as the immediate closure target because it stays on the existing Q8_0 lane and current Ubuntu evidence says the model can load and serve. The active blocker is tokenizer/template parity: the next status-changing work must land tokenizer/template fixtures, known-good prompt-token reference evidence, Camelid-vs-reference prompt-token comparison, bounded load/serve/readiness evidence, and fail-closed unsupported states before any generation/API/WebUI support language appears. Public docs should call this a planned exact-row bring-up only; phrases like “Mistral support” or “Mistral-family support” are premature until row-specific parity and smoke artifacts exist.

The first Mistral evidence bundle should include, at minimum: the exact `Mistral-7B-Instruct-v0.3.Q8_0.gguf` filename/SHA, GGUF metadata summary, tokenizer and chat-template fixture IDs, known-good prompt-token reference output, Camelid prompt-token comparison, load/serve/readiness result, memory notes, command transcript, scrubbed manifest, and checksum file. After that, generation parity, API smoke, frontend smoke, bounded context, and performance/portability evidence can be promoted one box at a time.
