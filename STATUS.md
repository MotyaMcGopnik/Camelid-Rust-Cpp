...
- **Active next-model bring-up set:** Camelid is now publicly working on four exact next-family rows, with one immediate closure lane and three follow-on candidates.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — immediate closure lane; Ubuntu load/serve evidence exists, but support remains fail-closed pending exact tokenizer/template references plus 1-token generation parity evidence.
...
6. Drive the first Mistral exact-row closure lane as planned work only: the chosen acceptance target is `Mistral-7B-Instruct-v0.3.Q8_0.gguf`; preserve the Ubuntu load/serve evidence, close exact tokenizer/template references plus 1-token generation parity, preserve fail-closed readiness behavior, and do not expose Mistral as supported until bounded row-specific artifacts exist.
