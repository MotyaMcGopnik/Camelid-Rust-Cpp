...
Executive summary: Camelid now has full API + frontend end-to-end smoke for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved, the 8B broader three-prompt 50-token rerun passed, and the first bounded 512-context pack now passes across all four exact Q8_0 rows. Llama 3.2 1B and Llama 3.2 3B now also have passing second bounded 1024-context parity packs, Llama 3.2 3B has a passing third bounded 2048-context parity pack, and the attempted Llama 3.2 1B third bounded 2048-context pack is explicitly red on first generated-token divergence despite prompt-token parity. The required 8B 1024-context diagnostic timed out in the backend before producing a parity report, so 8B 1024/2048-context promotion is paused and the lane shifts to performance/memory architecture work. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity envelopes; broad Llama-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, and portability remain outside the support claim.
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end short-context generation parity artifacts: compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, and bounded-memory evidence all agree for the exact tracked Q8_0 GGUF. The 8B 1024/2048 long-context boxes are blocked by backend timeout and are not promoted.
...
- `qa/evidence-bundles/llama32-3b-context-2048-20260505T105742Z-head-36ec8e492d65/manifest.json` records the third bounded 2048-context pack pass for the exact Llama 3.2 3B Instruct Q8_0 row from clean public head `36ec8e492d65`; the 1910-token prompt matched prompt tokens, generated token IDs `[34,2735,35,12,7854]`, and generated text `CMLD-204` for the checked 5-token pack. This closes only that exact 3B row+box slice and does not lend 2048-context support to 1B, 8B, model-native context, or broad Llama-family claims.
- `qa/validation-notes/2026-05-05-llama32-1b-context-2048-blocker.md` records the attempted third bounded 2048-context pack for the exact Llama 3.2 1B Instruct Q8_0 row: prompt tokens matched at 1910 tokens, but generated token/text parity failed at generated token index `0` (`The repeat marker is C` vs `CMLD-204`). This is an explicit blocker, not a PASS artifact.
- `qa/validation-notes/2026-05-05-llama32-1b-q8-2048-current-main-watchdog.md` records the clean-current-main watchdog rerun at head `7efa98bd6b3e7b4468177b0ff5aca1dd1634bc67`: prompt tokens still match at 1910 tokens, but generated token/text parity still fails at generated token index `0` (`[11,11,11,315,315]` / `",,, of of"` vs `[34,2735,35,12,7854]` / `"CMLD-204"`), with backend ranking the reference first token `34` at rank `1996`. This reconfirms the exact 1B/2048 red box and is not a PASS artifact.
- `qa/validation-notes/2026-05-05-llama32-1b-q8-2048-cache-probe.md` records the current-main first-token Q8 file-cache probe for the same exact 1B/2048 red box: a diagnostic 256 MiB Q8 file cache did not change the divergent first token (`[11]` vs `[34]`), left the reference token ranked `1996`, reduced only `516` read calls with essentially unchanged read bytes, and raised backend peak RSS from `464,468 KiB` to `817,512 KiB`. This keeps the path on structural Q8 read-reuse plus upstream layer/KV/attention correctness tracing, not cache-based promotion.
- `qa/validation-notes/2026-05-05-q8-borrowed-batch-read-reuse.md` records the follow-up structural Q8_0 read-reuse patch: borrowed/token-major file-backed Q8 matmul now routes eligible batches through the chunked Q8 block reader so each Q8 chunk is read once across input rows. This is local code evidence only and does not change any support row or 8B 1024/2048-context status.
- `qa/validation-notes/2026-05-05-llama32-1b-rope-freqs-runtime.md` records the follow-up runtime slice for that 1B 2048 blocker: Camelid now loads and prefers GGUF `rope_freqs.weight` when present, with local fmt/clippy/test coverage, but this is code evidence only until the 2048 prompt pack is rerun and produces a PASS artifact.
- `qa/validation-notes/2026-05-05-llama3-8b-context-1024-blocker.md` records the required first diagnostic from Tim's 8B operating rule for the exact Llama 3 8B Instruct Q8_0 row: llama.cpp accepted the 881-token 1024-context prompt, but Camelid `/v1/chat/completions` did not complete within the 900-second parity harness timeout, so no parity report was produced. This is an explicit blocker, not a PASS artifact, and pauses 8B long-context promotion.
- `qa/validation-notes/2026-05-05-llama3-8b-context-2048-blocker.md` records the attempted third bounded 2048-context pack for the exact Llama 3 8B Instruct Q8_0 row: llama.cpp accepted the 1910-token prompt and generated `CMLD-204`, but Camelid `/v1/chat/completions` did not complete within the 900-second parity harness timeout, so no parity report was produced. This is an explicit blocker, not a PASS artifact.
- Runtime/API alignment is now explicit for the exact Llama 3.2 1B/3B and Llama 3 8B context row+boxes: `/api/capabilities` exposes the 1024 track as `validated_second_pack` for the exact 1B/3B rows, exposes 1B 2048 as `blocked_first_token_divergence`, exposes 3B 2048 as `validated_third_pack`, and marks 8B 1024 and 8B 2048 as `blocked_backend_timeout_900s`; frontend API/Models surfaces display those row-specific tracks without lending them across rows.
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked 512-pack result only, not a 1k/2k/model-native context or portability claim. The exact Llama 3.2 1B and 3B rows also have separate checked 1024-context pack passes, the exact 3B row has a separate checked 2048-context pack pass, the exact 1B 2048-context attempt remains red on first-token divergence and has been reconfirmed red on clean current `main`, and the exact 8B 1024/2048-context attempts remain red on backend timeout; each remains limited to its row and prompt pack. A follow-up 1B runtime slice now prefers GGUF-provided RoPE frequencies, but it does not change the 2048 support status until the row has a fresh passing artifact.
...
- The attempted third bounded 2048-context pack is not green for this exact 1B row: prompt-token parity passed at 1910 prompt tokens, but generated-token/text parity failed at generated token index `0` (`The repeat marker is C` vs `CMLD-204`).
- Follow-up runtime code now loads and prefers GGUF `rope_freqs.weight` for RoPE when present, which targets the suspected long-context rotation mismatch; this is covered by local fmt/clippy/test gates only and does **not** promote 2048 context without a fresh passing Ubuntu parity artifact.
...
- The support claim is limited to this exact 1B Instruct Q8_0 row, short local-chat smoke, the checked first bounded 512-context pack, the checked second bounded 1024-context pack, the checked compact chat-template-shapes pack, and the bounded unique-chat perf/RSS envelope; neighboring Llama rows, other quantizations, the blocked 2048-context pack, model-native/larger contexts beyond checked packs, arbitrary GGUF/Jinja template execution, broad chat-template behavior, production throughput, and portability remain outside the claim.
...
- The required second bounded 1024-context diagnostic and attempted third bounded 2048-context pack are not green for this exact 8B row: llama.cpp accepted both bounded prompts, but Camelid `/v1/chat/completions` did not complete within the 900-second parity harness timeout, so no parity reports were produced.
- The current backend pivot is performance/memory architecture, not another 8B long-context promotion attempt: `qa/validation-notes/2026-05-05-prefill-chunk-runtime.md` records the chunked-prefill/Q8-read-reuse runtime slice, structured prefill memory instrumentation, local gates, and an isolated Ubuntu code-validation pass. This is code evidence only until row-specific Ubuntu timing/parity artifacts exist.
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope; neighboring Llama rows, other quantizations, the blocked 1024/2048-context packs, larger contexts, and broader chat-template behavior remain outside the claim.
