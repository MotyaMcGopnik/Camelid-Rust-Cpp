...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved and the 8B broader three-prompt 50-token rerun passed. Llama 3.2 1B/3B retain bounded 512/1024/2048-context packs where row-specific PASS bundles are cited; Llama 3 8B is promoted only through exact-row smoke plus the checked bounded 512-context pack on current `main`. The newest committed 8B 1024/2048 PASS at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T024342Z-head-b49034007f2e/manifest.json` is historical after later runtime/source/support-surface changes and does not close those buckets on current `main`; 1024/2048 stay red until a fresh current-head canonical PASS and docs/API/frontend alignment land. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. Mistral 7B Instruct v0.3 Q8_0 is active validation only: source/SHA, exact tokenizer/template references, 1-token generation parity, broader five-prompt/50-token parity, bounded 512/1024/2048, and checked 4096/8192 context evidence now exist, but support remains fail-closed until row-specific API/WebUI/RSS readiness and synchronized support surfaces land. The public support boundary moved only for the supported exact rows and only for the validated local-chat/parity envelopes; broad Llama/Mistral-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, production throughput, and portability remain outside the support claim.
...
| Llama 3 8B Instruct Q8_0 | The 8B row is no longer groundwork-only: compact parity, broader 50-token parity, API/WebUI, checked 512-context pack, compact template-shapes, memory evidence, structured Q8 read counters, and hot-path measurements are public. | 512 bounded pack is green for this exact row on current `main`; 1024/2048 are historical and stay red until a fresh current-head PASS plus docs/API/frontend alignment lands. | Hard boundary: no model-native/larger context beyond checked packs, broad 8B/Llama support, arbitrary templates, production throughput, portability, or neighboring-row/context promotion without row-specific evidence. |
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and the checked 512-context pack for the exact tracked Q8_0 GGUF. The newest committed 1024/2048 canonical pass at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T024342Z-head-b49034007f2e/manifest.json` matched prompt tokens, generated token IDs, and generated text (`CMLD-102` / `CMLD-204`) for source/runtime head `b49034007f2e`, but it is historical after later runtime/source/support-surface changes and does not promote current `main`; earlier `9e3c64f2cfab`, `160348118d44`, `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` passes remain historical source-head evidence.
...
- Current structural headroom work keeps layer-major prefill bounded while sizing scoped Q8 reuse carefully and bounding retained scratch. That code/test evidence does not by itself promote production throughput, broader/full support, or 8B 1024/2048; a fresh current-head 8B 1024/2048 canonical PASS plus docs/API/frontend alignment is still required before those exact checked packs can turn green again.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T024342Z-head-b49034007f2e/manifest.json` plus `SHA256SUMS` is the newest committed exact Llama 3 8B Q8_0 bounded 1024/2048 PASS bundle for source/runtime head `b49034007f2e`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`. It is historical after later runtime/source/support-surface changes and does not close current `main`; 1024/2048 remain red until fresh PASS artifacts plus docs/API/frontend alignment exist.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows, while the exact Llama 3 8B row is checked only through 512 on current `main`; 8B 1024/2048 remain red unless fresh PASS artifacts and docs/API/frontend alignment exist. These remain exact bounded-pack claims only.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote production throughput, broader/full support, or current-main 8B 1024/2048. A fresh current-head 8B 1024/2048 canonical PASS plus docs/API/frontend alignment is still required.
...
- The current 8B context support truth for this exact row includes the checked bounded 512 pack only. The newest committed 1024/2048 pass for source/runtime head `b49034007f2e` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T024342Z-head-b49034007f2e/manifest.json`, but it is historical after later runtime/source/support-surface changes; the earlier `9e3c64f2cfab`, `160348118d44`, `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` bundles remain historical source-head evidence. 8B 1024/2048 stay red on current `main` until a fresh canonical rerun plus docs/API/frontend alignment lands.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through the checked 512 bounded context pack on current `main`. Neighboring Llama rows, other quantizations, 8B 1024/2048, larger/model-native contexts beyond checked packs, production throughput, portability, arbitrary templates, and broader chat-template behavior remain outside the claim until their own evidence exists.
...
- the newest committed 1024/2048 bounded-context pack passed at source/runtime head `b49034007f2e` with prompt-token, generated-token, and generated-text parity; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T024342Z-head-b49034007f2e/`, but it is historical after later runtime/source/support-surface changes and does not promote current `main`; older 1024/2048 bundles remain historical for their source heads
...
This is a support promotion only for the exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope, including the checked bounded 512 context pack on current `main`; 1024/2048 remain unpromoted until fresh current-head evidence and aligned surfaces land. Broad Llama-family, model-native/larger-context beyond checked packs, production-throughput, portability, arbitrary-template, and neighboring-row support remain unpromoted.
...
4. Preserve the Llama 3 8B exact-row smoke plus checked-512 bounded-pack promotion in docs, API, frontend readiness, and regression evidence while keeping 1024/2048 red until fresh current-head evidence exists; do not lend any result to model-native/larger contexts or neighboring rows.
