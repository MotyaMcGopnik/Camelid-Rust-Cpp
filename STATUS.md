...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, Llama 3 8B Instruct Q8_0, and Mixtral-8x7B-Instruct-v0.1 Q8_0 rows within their checked envelopes, refreshed by historical Ubuntu-lane evidence. The Ubuntu validation lane is currently paused by operator instruction, so new promotion-grade runtime reruns are blocked until Tim explicitly reactivates a lane. The Mixtral row is exact-row supported after refreshed six-prompt 5-token parity, OpenAI-compatible API smoke, WebUI readiness, RSS/timing, and scrubbed manifest/checksum evidence passed. Llama 3.2 1B/3B retain bounded 512/1024/2048-context packs where row-specific PASS bundles are cited; Llama 3 8B is promoted through exact-row smoke plus checked bounded 512/1024/2048-context packs on current `main`. Mistral 7B Instruct v0.3 Q8_0 is active validation only: source/SHA, exact tokenizer/template references, 1-token generation parity, broader five-prompt/50-token parity, bounded 512/1024/2048, and checked 4096/8192 context evidence now exist, but support remains fail-closed until row-specific API/WebUI/RSS readiness and synchronized support surfaces land. The public support boundary moved only for the supported exact rows and only for the validated local-chat/parity envelopes; broad-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, production throughput, and portability remain outside the support claim.
...
| Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf | Exact-row support is green for the checked short-prompt MoE/API/WebUI/RSS envelope: six-prompt 5-token parity, API smoke, WebUI readiness, RSS/timing, and manifest/checksum bundles all passed. | Short-prompt max_tokens=5 only; no long-context bucket is promoted. | Hard boundary: no neighboring-row, long-context, arbitrary-template, production-throughput, portability, or broader/full claim without separate evidence. |

This is the repo story in one sentence: Camelid expanded from one trusted small-model gate to a five-row exact Q8_0 product contract, while preserving fail-closed support language for every unproven neighboring row, context, quantization, and runtime path.
...
- **Active next-model bring-up set:** Camelid is publicly working on exact next-family rows, with Mistral still fail-closed and Qwen/Gemma as follow-on candidates.
  ...
  - `Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` — exact-row supported for the checked short-prompt MoE/API/WebUI/RSS envelope; refreshed six-prompt 5-token parity, API smoke, WebUI readiness, RSS/timing, and manifest/checksum evidence are green.
...
Public readiness language is locked to row-by-row evidence: Mistral may say “in active validation, not supported yet”; the Mixtral row may say “exact-row supported” only inside the checked short-prompt MoE/API/WebUI/RSS envelope; Qwen and Gemma may say “planned exact-row candidate, not supported yet.” Any wider Mixtral-row claim requires separate long-context, broader-prompt, production-throughput, portability, scrubbed manifest, and checksum evidence.
...
Bottom line for reviewers: Camelid has the original TinyLlama verified support gate plus three exact Llama Q8_0 rows and one exact Mixtral Q8_0 row with verified support within validated bounds. That is a real end-to-end support expansion, but it is not a broad-family claim.
...
- `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-backend-parity-refresh-20260511/` records refreshed exact-row Mixtral six-prompt max_tokens=5 parity against the pinned split-tensor llama.cpp reference; prompt tokens, generated token IDs, and generated text matched for `Hello`, `What is 2+2?`, `Name a color.`, `Say yes.`, `Count to three.`, and `Write OK.`.
- `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-api-smoke-20260511/`, `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-webui-readiness-20260511/`, `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-rss-timing-runtime-20260511/`, and `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-manifest-checksum-20260511/` close the checked Mixtral exact-row API/WebUI/RSS/manifest gates; all are scrubbed exact-row evidence only.
...
7. Preserve `Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` as exact-row supported only inside the checked short-prompt MoE/API/WebUI/RSS envelope; next promotable work is long-context, broader prompt, throughput, portability, and repeated current-head evidence before any wider claim.
...
`Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` is now exact-row supported for the checked short-prompt MoE/API/WebUI/RSS envelope. The promotion evidence includes refreshed six-prompt max_tokens=5 parity against the pinned split-tensor llama.cpp reference, OpenAI-compatible API smoke, WebUI readiness, RSS/timing, and scrubbed manifest/checksum evidence. This is not a neighboring-row, long-context, arbitrary-template, production-throughput, portability, or broader/full claim.
