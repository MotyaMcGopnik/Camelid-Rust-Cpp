...
| Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf | Active validation unsupported with bounded one-token backend MoE runtime evidence: source/SHA/license, tokenizer/template references, lazy/file-backed rank-3 Q8 expert routing, and one-token smoke exist. | One-token backend runtime probe only; later short-prompt generation still diverges. | Hard boundary: no frontend/API/WebUI/RSS green state, neighboring-row, long-context, arbitrary-template, production-throughput, portability, or broader/full claim without separate evidence. |
...
- **Active next-model bring-up set:** Camelid is publicly working on exact next-family rows, with Mistral still fail-closed and Qwen/Gemma as follow-on candidates.
  ...
  - `Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` — active validation unsupported; bounded one-token backend MoE runtime evidence exists, but later short-prompt generation still diverges and API/WebUI/RSS/frontend support remains blocked.
...
Bottom line for reviewers: Camelid has the original TinyLlama verified support gate plus three exact Llama Q8_0 rows with verified support within validated bounds. Mixtral remains bounded one-token backend MoE runtime evidence only, not broad or frontend support.
...
- `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-support-probe-20260509/` is the bounded Mixtral evidence anchor: one-token backend MoE runtime evidence exists with lazy/file-backed rank-3 Q8 experts, but later short-prompt generation still diverges and no API/WebUI/RSS/frontend support gate is promoted.
...
7. Advance `Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` only inside the active-validation unsupported lane: metadata/tokenizer evidence, MoE top-k expert routing, and bounded one-token runtime evidence exist; later generation still diverges before any API/WebUI/support wording.
...
`Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` is not a supported row. It has bounded one-token backend MoE runtime evidence with lazy/file-backed rank-3 Q8 experts, but later short-prompt generation still diverges from llama.cpp. This is not an API/WebUI/frontend, neighboring-row, long-context, arbitrary-template, production-throughput, portability, or broader/full claim.
