...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved, the 8B broader three-prompt 50-token rerun passed, and the exact Llama 3.2 1B/3B rows have bounded 512/1024/2048-context packs green where listed. Llama 3 8B remains green through the checked bounded 512-context pack on current `main`; the 1024/2048 PASS bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260507T194559Z-head-ab8e465b50c3/manifest.json` is historical for head `ab8e465b50c3` after later Q8 runtime/source commits and must be rerun before current-head green wording returns. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity envelopes; broad Llama-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, and portability remain outside the support claim.
...
| Llama 3 8B Instruct Q8_0 | The 8B row is no longer groundwork-only: compact parity, broader 50-token parity, API/WebUI, the checked 512-context pack, historical 1024/2048 source-head PASS artifacts pending current-head rerun, compact template-shapes, memory evidence, and hot-path measurements are public. | 512 is green for this exact row on current `main`; the 1024/2048 PASS bundle is historical for head `ab8e465b50c3` after later Q8 runtime/source commits and needs a fresh canonical current-head rerun. | Hard boundary: no model-native/larger context beyond checked packs, broad 8B/Llama support, arbitrary templates, production throughput, or portability. |
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and a checked 512-context pack for the exact tracked Q8_0 GGUF. The older 1024/2048 bounded prompt-pack pass in `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260507T194559Z-head-ab8e465b50c3/manifest.json` remains source-head evidence only until rerun on current `main`.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260507T194559Z-head-ab8e465b50c3/manifest.json` plus `SHA256SUMS` is historical 8B 1024/2048 bounded-pack PASS evidence for `ab8e465b50c3`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`, but later Q8 runtime/source commits mean it is not current-head green evidence.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows only; the exact Llama 3 8B 1024/2048 buckets remain red until fresh current-head PASS artifacts and docs/API/frontend alignment exist.
...
- The current 8B context support truth for this exact row includes the checked 512 bounded pack. The 1024/2048 pass at `ab8e465b50c3` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260507T194559Z-head-ab8e465b50c3/manifest.json`, but it is historical after later Q8 runtime/source commits and requires a fresh canonical rerun before current-head green status. This is still exact-row bounded-pack support, not model-native or broad-family promotion.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through the checked 512 bounded context on current `main`; 1024/2048 remain pending current-head rerun; neighboring Llama rows, other quantizations, larger/model-native contexts beyond checked packs, and broader chat-template behavior remain outside the claim until their own evidence exists.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260507T194559Z-head-ab8e465b50c3/manifest.json`
