...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and does **not** promote 2048 context, 8B long-context support, or production throughput.
...
- The third bounded 2048-context pack is now green for this exact 8B row from clean current public `main`: the 1910-token prompt matched prompt tokens, generated token IDs `[34,2735,35,12,7854]`, generated text `CMLD-204`, wall time was `8:27.08`, timed-process max RSS was `17507392 KiB`, and max sampled backend RSS was `1537484 KiB`. The second bounded 1024-context pack is also green from clean current public `main`: the 881-token prompt matched prompt tokens, generated token IDs `[34,2735,35,12,4278]`, generated text `CMLD-102`, and wall time was `3:02.05`.
