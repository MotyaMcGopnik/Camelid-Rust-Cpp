...
- **Active next-model bring-up set:** Camelid is publicly working on exact next-family rows, with Mixtral/Qwen/Gemma as follow-on candidates.
  ...
  - **Gemma 4 lane:** `gemma-4-E2B-it-Q8_0.gguf` and `gemma-4-E4B-it-Q8_0.gguf` are supported exact-row text-token-generation smoke on the from-scratch `gemma4` engine — five-prompt greedy parity (prompt ids, generated ids, text) against the pinned llama.cpp `5d56eff` oracles (`qa/gemma4/oracle/`) on CPU AND the Metal GPU-resident runtime, **checked bounded 512/1024/2048/4096/8192-context packs (full-budget, no recorded frontiers, 20/20 CPU+GPU cells across the two rows)**, the reference-exact chat template locked by `qa/gemma4/template_shapes_v1.json`, API/streaming chat smoke behind `CAMELID_GEMMA4_SERVE`, and typed fail-closed multimodal rejection. Distributed layer sharding (`gemma4-master`/`gemma4-worker`, versioned handshake + checksummed packets) is parity-locked by `tests/gemma4_distributed_parity.rs`, and the distributed SERVE lane (`CAMELID_GEMMA4_WORKER`/`CAMELID_GEMMA4_SPLIT`) is loopback-validated. `gemma-4-12b-it-Q8_0.gguf` is active validation (single-node CPU memory-bound on 16 GB; the two-Mac distributed lane is its target — two-Mac CLI parity is proven, and the distributed SERVE lane now answers chat/completions/SSE end-to-end across the pair: bundle `gemma4-12b-it-q8-0-distributed-serve-20260610T235155Z`; promotion still needs WebUI closure). The QAT Q4_0/Q6_K CPU wire lane exists with first-light generation on `gemma-4-E4B_q4_0-it.gguf` (no oracle parity claimed yet); 26B A4B / 31B / MTP rows fail closed with named blockers — see `docs/recon/GEMMA4_26B_A4B_QAT_RECON.md` for the 26B QAT work list.
