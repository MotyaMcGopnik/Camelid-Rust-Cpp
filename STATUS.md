...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts: compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and checked 512/1024/2048-context packs all agree for the exact tracked Q8_0 GGUF. The 8B 1024 and 2048 bounded context boxes are promoted only for those exact checked packs.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260507T063016Z-head-48c2b8d4ba1f/manifest.json` plus `SHA256SUMS` records the fresh current-head 8B 1024/2048 bounded-pack pass for the exact Llama 3 8B Instruct Q8_0 row; older 1024/2048 attempts stay review history.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows and the exact Llama 3 8B row; frontend surfaces must display that bounded-only boundary.
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked 512-pack result only, not a 1k/2k/model-native context or portability claim. The exact Llama 3.2 1B and 3B rows also have separate checked 1024-context pack passes, and both exact Llama 3.2 rows have separate checked 2048-context pack passes. The exact 8B 1024/2048 buckets are now promoted only as checked bounded-pack passes.
...
- The current 8B context support truth for this exact row includes checked 512/1024/2048 bounded packs only.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through checked 512/1024/2048 bounded contexts; neighboring Llama rows, other quantizations, larger/model-native contexts beyond checked packs, and broader chat-template behavior remain outside the claim.
