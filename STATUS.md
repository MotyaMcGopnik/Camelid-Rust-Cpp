...
Executive summary: Camelid has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved and the 8B broader three-prompt 50-token rerun passed. Llama 3.2 1B/3B retain bounded 512/1024/2048-context packs where row-specific PASS bundles are cited; Llama 3 8B remains limited to the committed checked bounded 512-context pack. The latest committed 8B 1024/2048 bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T210500Z-head-160348118d44/manifest.json` passed for source/runtime head `160348118d44`, but later commits make it historical evidence rather than a current-head green claim. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. Mistral 7B Instruct v0.3 Q8_0 is active validation only: source/SHA, exact tokenizer/template references, 1-token generation parity, bounded context, and broader five-prompt/50-token parity evidence now exist, but support remains fail-closed until row-specific API/WebUI/RSS readiness and synchronized support surfaces land. The public support boundary moved only for the supported exact rows and only for the validated local-chat/parity envelopes; broad Llama/Mistral-family support, current-head 8B 1024/2048 support, model-native/larger contexts beyond the checked packs, arbitrary template execution, production throughput, and portability remain outside the support claim.
...
| Llama 3 8B Instruct Q8_0 | The 8B row is no longer groundwork-only: compact parity, broader 50-token parity, API/WebUI, the checked 512-context pack, compact template-shapes, memory evidence, structured Q8 read counters, and hot-path measurements are public. | The 512 bounded pack is committed evidence; 1024/2048 PASS bundles are historical for their recorded source/runtime heads until a fresh current-head PASS plus docs/API/frontend alignment lands. | Hard boundary: no current-head 1024/2048, model-native/larger context beyond checked packs, broad 8B/Llama support, arbitrary templates, production throughput, portability, or neighboring-row/context promotion without row-specific evidence. |
...
- **Active next-model bring-up set:** Camelid is publicly working on four exact next-family rows, with Mistral first but still fail-closed and three follow-on candidates.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — immediate closure lane; tokenizer/template, 1-token generation, bounded context, and broader five-prompt/50-token parity evidence are green, but support remains fail-closed pending row-specific API/WebUI/RSS evidence and synchronized support surfaces.
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and the checked 512-context pack for the exact tracked Q8_0 GGUF. The latest committed 1024/2048 bounded prompt-pack pass at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T210500Z-head-160348118d44/manifest.json` covers source/runtime head `160348118d44` only; current-head 1024/2048 remains unpromoted until rerun and aligned, and earlier `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` passes remain historical source-head evidence.
...
- Current structural headroom work keeps layer-major prefill bounded while sizing the scoped Q8 file cache to at least the largest lazy-Q8 layer working set and reusing the chunk input buffer across layers. This is local code/test evidence only and does not promote 8B 1024/2048, production throughput, or broader/full support.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T210500Z-head-160348118d44/manifest.json` plus `SHA256SUMS` is the latest committed exact Llama 3 8B Q8_0 bounded 1024/2048 PASS bundle for source/runtime head `160348118d44`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`. It is historical evidence after later commits and does not promote current-head 1024/2048, model-native/larger context, arbitrary templates, neighboring rows, broad 8B/Llama-family support, portability, or production throughput.
- `qa/evidence-bundles/mistral-7b-v0.3-q8-broader-50tok-ubuntu-20260509T000633Z-head-d330e97ae992/manifest.json` plus `SHA256SUMS` records the exact Mistral 7B Instruct v0.3 Q8_0 broader five-prompt/50-token Ubuntu parity pass: prompt tokens, generated token IDs, and generated text all matched llama.cpp. Mistral remains active validation only; API/WebUI/RSS readiness and synchronized support surfaces are still required before any support-promotion evidence anchor can be cited.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows where row-specific PASS artifacts exist, while the exact Llama 3 8B row exposes only the checked 512 pack; 8B 1024/2048 require fresh current-head PASS artifacts plus docs/API/frontend alignment.
...
- Llama 3.2 1B Q8_0 moved from evidence-only to supported exact-row smoke after compact parity, broader prompt-pack parity, `/api/models/load`, `/v1/completions`, `/v1/chat/completions`, and frontend smoke evidence aligned. Its bounded compact chat-template-shapes box, bounded unique-chat perf/RSS envelope box, second bounded 1024-context parity box, and third bounded 2048-context parity box after the RoPE frequency-factor fix are now green for the exact row only.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. Current-head 8B 1024/2048 still requires a fresh canonical PASS bundle after later source changes.
...
- The current 8B context support truth for this exact row includes the committed checked 512 bounded pack. The latest committed 1024/2048 pass for source/runtime head `160348118d44` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T210500Z-head-160348118d44/manifest.json`, but current-head 1024/2048 remains unpromoted until rerun and aligned; the earlier `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` bundles remain historical source-head evidence.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through the committed checked 512 bounded context pack; 1024/2048 are historical source-head evidence until rerun and aligned. Neighboring Llama rows, other quantizations, larger/model-native contexts beyond checked packs, production throughput, portability, arbitrary templates, and broader chat-template behavior remain outside the claim until their own evidence exists.
...
- the latest committed 1024/2048 bounded-context pack passed at source/runtime head `160348118d44` with prompt-token, generated-token, and generated-text parity; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T210500Z-head-160348118d44/`; current-head 1024/2048 remains unpromoted and older 1024/2048 bundles remain historical for their source heads
...
This is a support promotion only for the exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope, including the committed checked bounded 512 context pack. Current-head 1024/2048, broad Llama-family, model-native/larger-context, production-throughput, portability, arbitrary-template, and neighboring-row support remain unpromoted.
...
4. Preserve the Llama 3 8B exact-row smoke plus checked-512 promotion in docs, API, frontend readiness, and regression evidence without lending it to 1024/2048 or neighboring rows until fresh evidence exists.
...
6. Advance the first Mistral exact-row bring-up lane without promotion: keep `Mistral-7B-Instruct-v0.3.Q8_0.gguf` fail-closed until exact tokenizer/template references plus 1-token generation parity evidence exist, then require separate API/WebUI/RSS evidence before any support wording.
...
Mistral work is open as the first exact-row bring-up track, but it has no validated release evidence and no runtime support claim yet. Camelid has locked `Mistral-7B-Instruct-v0.3.Q8_0.gguf` as the immediate closure target because it stays on the existing Q8_0 lane and current Ubuntu evidence says the model can load and serve. The active blocker is support-grade row evidence: the next status-changing work must land exact tokenizer/template fixtures, known-good prompt-token reference evidence, Camelid-vs-reference prompt-token comparison, 1-token generation parity, bounded load/serve/readiness evidence, and fail-closed unsupported states before any generation/API/WebUI support language appears. Public docs should call this a planned exact-row bring-up only; phrases like “Mistral support” or “Mistral-family support” are premature until row-specific parity and smoke artifacts exist.

The first Mistral evidence bundle should include, at minimum: the exact `Mistral-7B-Instruct-v0.3.Q8_0.gguf` filename/SHA, GGUF metadata summary, tokenizer and chat-template fixture IDs, known-good prompt-token reference output, Camelid prompt-token comparison, 1-token generation parity, load/serve/readiness result, memory notes, command transcript, scrubbed manifest, and checksum file. After that, API smoke, frontend smoke, bounded context, and performance/portability evidence can be promoted one box at a time.
