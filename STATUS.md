...
- The support claim is limited to this exact 3B Instruct Q8_0 row, the validated local-chat/parity envelope, the checked first bounded 512-context pack, the checked compact chat-template-shapes pack, and the bounded unique-chat perf/RSS envelope; larger/broader contexts, portable packaging, arbitrary GGUF/Jinja template execution, broader chat-template coverage, production throughput, and portability remain follow-up gates.
...
Expansion beyond the current supported row remains blocked until Camelid has larger/broader context coverage beyond the checked 512 pack plus production performance, portability, and arbitrary/Jinja template evidence for this exact row.
