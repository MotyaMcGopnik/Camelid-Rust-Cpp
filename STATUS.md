...
- `qa/evidence-bundles/llama3-8b-api-webui-rss-20260505T014408Z-head-8cef7af4d6c6/manifest.json` records a reopened-lane exact 8B API/WebUI smoke pass for the completion-diagnostics API patch over `8cef7af`: `/v1/completions` now carries response-local timing diagnostics, the promotion smoke timing summary passed, frontend chat generated `Hello`, and RSS was sampled around the smoke window. This is a patched-tree API evidence slice, not a new broad support claim.
...
- patched-tree API timing diagnostics for `/v1/completions` passed the exact 8B API/WebUI smoke and response-local timing summary at `qa/evidence-bundles/llama3-8b-api-webui-rss-20260505T014408Z-head-8cef7af4d6c6/manifest.json`; sampled backend RSS for that smoke moved `6,308 -> 286,216 KiB`, which is useful smoke-window evidence but not peak memory proof
