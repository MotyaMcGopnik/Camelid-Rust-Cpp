...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and checked 512/1024/2048-context packs for the exact tracked Q8_0 GGUF. The current-head 1024/2048 canonical pass at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T002636Z-head-9e3c64f2cfab/manifest.json` matched prompt tokens, generated token IDs, and generated text (`CMLD-102` / `CMLD-204`) for source/runtime head `9e3c64f2cfab`; earlier `160348118d44`, `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` passes remain historical source-head evidence.
...
- Current structural headroom work keeps layer-major prefill bounded while sizing the scoped Q8 file cache to at least the largest lazy-Q8 layer working set and reusing the chunk input buffer across layers. That code/test evidence did not by itself promote production throughput or broader/full support; the separate current-head 8B 1024/2048 canonical PASS bundle below closes only the exact checked packs.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T002636Z-head-9e3c64f2cfab/manifest.json` plus `SHA256SUMS` is the latest committed exact Llama 3 8B Q8_0 bounded 1024/2048 current-head PASS bundle for source/runtime head `9e3c64f2cfab`; it matched prompt tokens, generated token IDs, and generated text for `CMLD-102` and `CMLD-204`. It closes only those exact checked packs for the exact 8B row and does not promote model-native/larger context beyond checked packs, arbitrary templates, neighboring rows, broad 8B/Llama-family support, portability, or production throughput.
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T210500Z-head-160348118d44/manifest.json` plus `SHA256SUMS` is the previous committed exact Llama 3 8B Q8_0 bounded 1024/2048 PASS bundle for source/runtime head `160348118d44`; it remains historical source-head evidence after later commits.
...
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows and the exact Llama 3 8B row where row-specific PASS artifacts exist. These remain exact bounded-pack claims only.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote production throughput or broader/full support. The separate current-head 8B 1024/2048 canonical PASS bundle closes only the exact checked packs.
...
- The current 8B context support truth for this exact row includes checked bounded 512/1024/2048 packs. The current-head 1024/2048 pass for source/runtime head `9e3c64f2cfab` matched the bounded prompt packs exactly and is published at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T002636Z-head-9e3c64f2cfab/manifest.json`; the earlier `160348118d44`, `9f8588bb4a4e`, `844e8d4709d1`, `86081876d01e`, and `e203d3cf3ea5` bundles remain historical source-head evidence.
...
- the current-head 1024/2048 bounded-context pack passed at source/runtime head `9e3c64f2cfab` with prompt-token, generated-token, and generated-text parity; public summary/checksums live at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T002636Z-head-9e3c64f2cfab/`; older 1024/2048 bundles remain historical for their source heads
...
This is a support promotion only for the exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope, including checked bounded 512/1024/2048 context packs. Broad Llama-family, model-native/larger-context beyond checked packs, production-throughput, portability, arbitrary-template, and neighboring-row support remain unpromoted.
...
4. Preserve the Llama 3 8B exact-row smoke plus checked-512/1024/2048 bounded-pack promotion in docs, API, frontend readiness, and regression evidence without lending it to model-native/larger contexts or neighboring rows until fresh evidence exists.
