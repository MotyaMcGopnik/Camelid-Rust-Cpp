...
| Llama 3.2 1B Instruct Q8_0 | Exact-row verified support moved beyond a demo: API, WebUI, compact/broader parity, bounded template-shapes, unique-chat RSS/perf, and post-fix 2048 evidence all agree. | 512, 1024, 2048, and 4096 bounded packs are green; 2048 is green only after the RoPE frequency-factor fix, and 4096 is green on current head `039353c38b52` for the compact-template recall pack. | Broader/full support still needs model-native/larger context, arbitrary/Jinja template coverage, production throughput, portability, and durable full-support normalization. |
...
- `qa/evidence-bundles/llama32-1b-context-4096-current-head-20260513T155455Z-head-039353c/manifest.json` records the fourth bounded 4096-context compact-template pack pass for the exact Llama 3.2 1B Instruct Q8_0 row on current head `039353c38b52`: the 3755-token prompt matched prompt tokens, generated token IDs `[34,2735,35,12,12378]`, and generated text `CMLD-409` for the checked 5-token pack. This closes only that exact 1B row+4096 box slice, not neighboring rows or model-native/larger context beyond checked packs.
...
- The support claim is limited to this exact 1B Instruct Q8_0 row, short local-chat smoke, the checked first bounded 512-context pack, the checked second bounded 1024-context pack, the checked third bounded 2048-context pack, the checked fourth bounded 4096-context pack, the checked compact chat-template-shapes pack, and the bounded unique-chat perf/RSS envelope; neighboring Llama rows, other quantizations, model-native/larger contexts beyond checked packs, arbitrary GGUF/Jinja template execution, broad chat-template behavior, production throughput, and portability remain outside the claim.
...
- `qa/evidence-bundles/llama32-1b-context-4096-current-head-20260513T155455Z-head-039353c/manifest.json`
