...
Executive summary: Camelid now has full API + frontend end-to-end smoke for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved, the 8B broader three-prompt 50-token rerun passed, and the first bounded 512-context pack now passes across all four exact Q8_0 rows. Llama 3.2 1B and Llama 3.2 3B now also have passing second bounded 1024-context parity packs, and both exact Llama 3.2 rows have passing third bounded 2048-context parity packs; the 1B 2048 box turned green only after the RoPE frequency-factor fix. The 8B long-context blocker is now narrowed: clean current-main `e146d3b` 1024-context and 2048-context proof passes produced public scrubbed parity artifacts, so both exact 8B bounded packs are promoted. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity envelopes; broad Llama-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, and portability remain outside the support claim.
...
| Llama 3.2 1B Instruct Q8_0 | Exact-row smoke moved beyond a demo: API, WebUI, compact/broader parity, bounded template-shapes, unique-chat RSS/perf, and post-fix 2048 evidence all agree. | 512, 1024, and 2048 bounded packs are green; 2048 is green only after the RoPE frequency-factor fix. | Broader/full support still needs model-native/larger context, arbitrary/Jinja template coverage, production throughput, portability, and durable full-support normalization. |
...
| Llama 3 8B Instruct Q8_0 | The 8B row is no longer groundwork-only: compact parity, broader 50-token parity, API/WebUI, 512-context, clean-current-main 1024/2048-context, compact template-shapes, memory evidence, and hot-path measurements are public. | 512, 1024, and 2048 bounded packs are green for this exact row. | Hard boundary: the 1024/2048 passes are bounded exact-row pack claims only, not model-native/larger context, broad 8B/Llama support, arbitrary templates, production throughput, or portability. |
...
- `qa/validation-notes/2026-05-05-llama3-8b-context-1024-blocker.md` records the earlier required 1024 diagnostic timeout for the exact Llama 3 8B Instruct Q8_0 row. It is superseded by the clean-current-main public PASS bundle below; keep it as red-box history, not current support truth.
- `qa/validation-notes/2026-05-05-llama3-8b-context-2048-blocker.md` records the earlier failed 2048 attempt and is now superseded by the clean-current-main public PASS bundle below; keep it as red-box history, not current support truth.
- `qa/evidence-bundles/llama3-8b-context-1024-20260506T182100Z-head-e146d3b335d8/manifest.json` records the clean current-main second bounded 1024-context pack pass for the exact Llama 3 8B Instruct Q8_0 row: 881 prompt tokens, generated token IDs `[34,2735,35,12,4278]`, generated text `CMLD-102`, `2:06.07` wall time, and timed-process max RSS `17372872 KiB`. The bundle is scrubbed and closes only this exact bounded 1024 pack.
- `qa/evidence-bundles/llama3-8b-context-2048-20260506T182534Z-head-e146d3b335d8/manifest.json` records the clean current-main third bounded 2048-context pack pass for the exact Llama 3 8B Instruct Q8_0 row: 1910 prompt tokens, generated token IDs `[34,2735,35,12,7854]`, generated text `CMLD-204`, `4:33.81` wall time, timed-process max RSS `17506972 KiB`, and max sampled backend RSS `1847360 KiB`. The bundle is scrubbed and closes only this exact bounded 2048 pack.
- Runtime/API alignment is now explicit for the exact Llama 3.2 1B/3B and Llama 3 8B context row+boxes: `/api/capabilities` exposes the 1024 track as `validated_second_pack` for the exact 1B/3B/8B rows and exposes 1B/3B/8B 2048 as `validated_third_pack`; frontend API/Models surfaces display those row-specific tracks without lending them across rows.
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked 512-pack result only, not a 1k/2k/model-native context or portability claim. The exact Llama 3.2 1B and 3B rows also have separate checked 1024-context pack passes, both exact Llama 3.2 rows have separate checked 2048-context pack passes, the exact 8B 1024- and 2048-context packs now have clean-current-main public PASS bundles; each remains limited to its row and prompt pack.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and did **not by itself** promote context, 8B long-context support, or production throughput. Later 8B 1024/2048 bounded-pack support comes only from the clean-current-main PASS bundles.
...
Expansion beyond the current supported row remains blocked until Camelid has model-native/larger context coverage beyond the checked 512/1024/2048 packs plus production performance, portability, and arbitrary/Jinja template evidence for this exact row.
...
- The third bounded 2048-context pack is now green for this exact 8B row from clean current public `main`: the 1910-token prompt matched prompt tokens, generated token IDs `[34,2735,35,12,7854]`, generated text `CMLD-204`, wall time was `4:33.81`, timed-process max RSS was `17506972 KiB`, and max sampled backend RSS was `1847360 KiB`. The second bounded 1024-context pack is also green from clean current public `main`: the 881-token prompt matched prompt tokens, generated token IDs `[34,2735,35,12,4278]`, generated text `CMLD-102`, wall time was `2:06.07`, and timed-process max RSS was `17372872 KiB`.
- The remaining backend pivot after this bounded 2048 promotion is performance/memory architecture follow-up: `qa/validation-notes/2026-05-05-prefill-chunk-runtime.md` records the chunked-prefill/Q8-read-reuse runtime slice, structured prefill memory instrumentation, local gates, and an isolated Ubuntu code-validation pass. This is code evidence only until row-specific Ubuntu timing/parity artifacts exist.
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope; neighboring Llama rows, other quantizations, larger/model-native contexts beyond the checked 1024/2048 packs, and broader chat-template behavior remain outside the claim.
...
- `qa/evidence-bundles/llama3-8b-context-1024-20260506T182100Z-head-e146d3b335d8/manifest.json`
- `qa/evidence-bundles/llama3-8b-context-2048-20260506T182534Z-head-e146d3b335d8/manifest.json`
