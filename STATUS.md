...
Executive summary: Camelid now has full API + frontend end-to-end validation for the exact Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Instruct Q8_0 rows, refreshed on the reopened Ubuntu validation lane. The previous 3B JSON-shaped broader prompt-pack blocker is resolved, the 8B broader three-prompt 50-token rerun passed, and the first bounded 512-context pack now passes across all four exact Q8_0 rows. Llama 3.2 1B and Llama 3.2 3B now also have passing second bounded 1024-context parity packs, and both exact Llama 3.2 rows have passing third bounded 2048-context parity packs; the 1B 2048 box turned green only after the RoPE frequency-factor fix. Llama 3 8B 1024/2048 remain red until fresh current-head PASS artifacts and synchronized docs/API/frontend copy exist. Llama 3.2 1B/3B bounded compact chat-template-shapes and bounded unique-chat perf/RSS envelopes have passing public summaries, and compact 8B chat-template-shapes also has a passing public summary. The public support boundary moved only for those exact rows and only for the validated local-chat/parity envelopes; broad Llama-family support, model-native/larger contexts beyond the checked packs, arbitrary template execution, and portability remain outside the support claim.
...
- **8B promotion:** Llama 3 8B Instruct Q8_0 has end-to-end bounded generation parity artifacts for compact parity, a three-prompt 50-token Ubuntu parity run, the bounded compact chat-template-shapes pack, API/frontend smoke, bounded-memory evidence, and the checked 512-context pack for the exact tracked Q8_0 GGUF. The 8B 1024 and 2048 bounded context boxes remain red until fresh current-head PASS artifacts and synchronized docs/API/frontend copy exist.
...
- `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260507T075407Z-head-4191ef9fcc28/manifest.json` plus `SHA256SUMS` is retained as historical 8B 1024/2048 bounded-pack evidence, but it is not promoted at the current public head; older 1024/2048 attempts stay review history.
- `qa/evidence-bundles/llama3-8b-context-1024-20260506T182100Z-head-e146d3b335d8/manifest.json` and `qa/evidence-bundles/llama3-8b-context-2048-20260506T182534Z-head-e146d3b335d8/manifest.json` remain superseded review artifacts.
- Runtime/API alignment is now explicit: `/api/capabilities` exposes 1024/2048 as promoted for the exact Llama 3.2 1B/3B rows only; exact Llama 3 8B exposes 512 as green and 1024/2048 as red pending fresh current-head PASS artifacts plus synchronized docs/API/frontend copy.
...
- The first bounded 512-context pack now passes across all four exact supported Q8_0 rows; this is a checked 512-pack result only, not a 1k/2k/model-native context or portability claim. The exact Llama 3.2 1B and 3B rows also have separate checked 1024-context pack passes, and both exact Llama 3.2 rows have separate checked 2048-context pack passes. The exact 8B 1024/2048 buckets remain red until fresh current-head PASS artifacts and synchronized docs/API/frontend copy exist.
...
- Follow-up structural Q8 code now reuses file-backed borrowed/token-major Q8 chunks across batched input rows when the weight layout can use the existing chunked Q8 block reader; this is covered by local guardrails only and does **not by itself** promote context, 8B long-context support, or production throughput; the later current-head 8B 1024/2048 evidence bundle is the bounded-pack promotion artifact.
...
- The current 8B context support truth for this exact row includes the checked 512 bounded pack only; 1024/2048 remain red until fresh current-head PASS artifacts and synchronized docs/API/frontend copy exist.
...
- The support claim is limited to this exact Llama 3 8B Instruct Q8_0 row and tested smoke/parity envelope through the checked 512 bounded context; neighboring Llama rows, other quantizations, 8B 1024/2048, larger/model-native contexts beyond checked packs, and broader chat-template behavior remain outside the claim until their own current-head evidence and surface alignment exist.
