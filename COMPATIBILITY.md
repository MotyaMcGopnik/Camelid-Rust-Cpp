...
- `qa/evidence-bundles/four-row-api-webui-20260504T233202Z-head-cb8943d/manifest.json` is the latest reopened-lane API + frontend smoke summary for all four exact rows; `qa/evidence-bundles/four-row-api-only-20260504T230722Z-head-13a465608fbf/manifest.json` is the narrower API-only predecessor.
...
Reopened-lane API + frontend smoke: `qa/evidence-bundles/four-row-api-webui-20260504T233202Z-head-cb8943d/manifest.json` records passing exact-row release build, frontend build/model-state smoke, API load/models/capabilities/completions/chat checks, and frontend smoke for TinyLlama, Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Q8_0. Treat it as freshness evidence for the exact smoke rows only; it does not replace parity packs, performance portability, or broader context validation. The earlier API-only summary remains at `qa/evidence-bundles/four-row-api-only-20260504T230722Z-head-13a465608fbf/manifest.json`, and the follow-up 8B context pack at `qa/evidence-bundles/llama3-8b-context-512-20260504T234625Z-head-58acf592345c/manifest.json` closes only the first bounded 512-context timeout for that exact 8B pack.
