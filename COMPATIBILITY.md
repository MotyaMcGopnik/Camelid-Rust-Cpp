...
- `qa/evidence-bundles/8b-checkmark-current-head-20260505T052647Z-head-864e07b51f36/manifest.json` plus `SHA256SUMS` is the latest public-main 8B exact-row checkmark refresh; API/WebUI/RSS smoke passed at `864e07b51f36`, frontend chat remained enabled, and max sampled backend RSS was `286056 KiB` without broadening beyond exact-row smoke support.
...
| Llama 3.2 3B Instruct Q8_0 | Supported exact-row smoke | Exact-row load, compact prompt-token/1-token/5-token/50-token parity, broader three-prompt 50-token parity, API smoke, frontend smoke, the first bounded 512-context pack, the bounded compact chat-template-shapes pack, and the bounded unique-chat perf/RSS envelope are supported for this exact row only. | Normalize broader/larger context, arbitrary/Jinja template behavior beyond the checked compact pack, production throughput, portability, and durable bundle evidence to the TinyLlama bar before any broader/full-support claim. |
...
- **Family / quant:** LLaMA decoder + Llama 3 BPE, Q8_0
- **Validated now:** the exact tracked GGUF has validated metadata/load, compact prompt-token plus deterministic 1-token/5-token/50-token parity, broader three-prompt 50-token parity, the first bounded 512-context pack, the bounded compact chat-template-shapes pack, the bounded unique-chat perf/RSS envelope, `/v1/completions`, `/v1/chat/completions`, frontend smoke, and a five-prompt API smoke pack
- **Missing gates:** broader/larger context beyond the checked 512 pack, production throughput evidence, portable packaging, and arbitrary/Jinja template coverage beyond the checked compact pack
...
- **Phase 11 — Llama 3 / Backend + QA:** keep broad Llama 3 below support until each concrete target has the right artifact-backed evidence. For Llama 3.2 1B Q8_0, exact-row short-chat smoke support is now strengthened by a passing three-prompt llama.cpp pack in the downloaded-model matrix and the first bounded 512-context pack. For Llama 3.2 3B Q8_0, exact-row smoke support is now strengthened by compact parity, API, frontend, smoke-pack evidence, the post-Q8-dot clean broader three-prompt 50-token parity rerun, and the first bounded 512-context pack; larger-context, performance/portability, and broader chat-template evidence remain the next requirements before widening the row. For Llama 3 8B, compact prompt-token, 1-token, 5-token, bounded 50-token parity, the three-prompt 50-token broader pass, basic API/frontend smoke, the first bounded 512-context pack, and bounded memory evidence now support the exact row; the next Backend/Performance action is to broaden prompt-pack breadth, 1k/2k context, chat-template coverage, performance, and portability evidence, preserving the serial/block-aligned determinism guardrails until optimized kernels have separate zero-delta evidence.
