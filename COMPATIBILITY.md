...
- **Phase 13 — Context/KV / QA + Backend:** audit 512, 1k, and 2k context buckets only after the host/runtime lane is reopened and the exact-row commands can run on an approved validation host; publish per-target tested context limits here and in readiness/API copy only after passing artifacts exist.
