...
Reopened-lane API + frontend smoke: `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json` records passing exact-row release build, frontend build/model-state smoke, API load/models/capabilities/completions/chat checks, and frontend smoke for TinyLlama, Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B Q8_0 on clean public `main` head `b403884`. Treat it as freshness evidence for the exact smoke rows only; it does not replace parity packs, performance portability, or broader context validation. The earlier API-only summary remains at `qa/evidence-bundles/four-row-api-only-20260504T230722Z-head-13a465608fbf/manifest.json`; the 1B/3B chat-template-shapes pack at `qa/evidence-bundles/llama32-1b-3b-chat-template-shapes-20260505T060036Z-head-e9f28572e090/manifest.json` closes only the bounded compact template-shape pack for those two exact rows; the 1B/3B unique-chat perf/RSS pack at `qa/evidence-bundles/llama32-1b-3b-unique-chat-perf-rss-20260505T061644Z-head-e9f28572e090/manifest.json` closes only the bounded memory/perf envelope box for those exact rows; the exact 3B parallel Q8 first-token bundle at `qa/evidence-bundles/llama32-3b-parallel-q8-first-token-20260505T140400Z-head-ffc22b85214f/manifest.json` closes only the opt-in parallel Q8 first-token runtime direction sub-box for that exact row; the 1B second bounded 1024-context pack at `qa/evidence-bundles/llama32-1b-context-1024-20260505T081001Z-head-156ded6fc76b/manifest.json` closes only that exact 1B context box; the 3B second bounded 1024-context pack at `qa/evidence-bundles/llama32-3b-context-1024-20260505T094258Z-head-c14e5e7b5692/manifest.json` and third bounded 2048-context pack at `qa/evidence-bundles/llama32-3b-context-2048-20260505T105742Z-head-36ec8e492d65/manifest.json` close only those exact 3B context boxes; the follow-up 8B broader pack at `qa/evidence-bundles/llama3-8b-broader-50tok-20260505T005031Z-head-d13541ad8d7e/manifest.json` closes only the three-prompt 50-token pack; the four-row context pack at `qa/evidence-bundles/four-row-context-512-20260505T051510Z-head-b403884/manifest.json` closes only the first bounded 512-context pack for the four exact Q8_0 rows; the earlier 8B context pack at `qa/evidence-bundles/llama3-8b-context-512-20260504T234625Z-head-58acf592345c/manifest.json` closes only the exact 8B first bounded 512-context timeout; the 8B chat-template-shapes pack at `qa/evidence-bundles/llama3-8b-chat-template-shapes-20260505T003821Z-head-d13541ad8d7e/manifest.json` closes only the bounded compact template-shape pack; the retained-block lazy-Q8 hot-path bundle at `qa/evidence-bundles/llama3-8b-lazy-q8-hotpath-20260505T021411Z-head-723a665/manifest.json` measures representative costs only for that exact 8B row; `qa/validation-notes/2026-05-05-llama3-8b-context-1024-blocker.md` records the required 1024 diagnostic timeout, and `qa/validation-notes/2026-05-05-llama3-8b-context-2048-blocker.md` records a blocked 2048-context attempt; both are blockers, not PASS artifacts.
...
| Llama 3.2 1B Instruct Q8_0 | Supported exact-row smoke | Exact-row load, compact/broader parity, API smoke, frontend smoke, the first bounded 512-context pack, the second bounded 1024-context pack, the bounded compact chat-template-shapes pack, and the bounded unique-chat perf/RSS envelope are supported for this exact row only; the attempted third bounded 2048-context pack is blocked by first generated-token divergence despite prompt-token parity. | Debug the 2048-context first-token divergence, then normalize model-native/larger context beyond the checked packs, arbitrary/Jinja template behavior beyond the checked compact pack, production throughput, portability, and durable bundle evidence to the TinyLlama bar before any broader/full-support claim. |
...
| Llama 3 8B Instruct Q8_0 | Supported exact-row smoke | Compact parity, three-prompt 50-token parity, API/frontend smoke, bounded-memory evidence, the first bounded 512-context pack, the bounded compact chat-template-shapes pack, and measurement-only lazy-Q8 hot-path cost probes support this exact row only. The 1024/2048 context packs are blocked by backend timeout. | Pause 8B long-context promotion and shift to KV/cache and attention memory behavior, Q8 matmul/output-projection hot paths, dense/f32 materialization avoidance, chunked/streamed long-context prefill, and per-layer timing/RSS-peak instrumentation with TinyLlama/1B/3B short-context guardrails. The 512-context and compact template-shape gaps are cleared only for the checked packs, and the hot-path probe is not a portability claim. |
...
- **Blocked now:** the attempted third bounded 2048-context pack has exact prompt-token parity but fails generated-token/text parity at generated token index `0`; a clean current-main watchdog rerun at head `7efa98bd6b3e7b4468177b0ff5aca1dd1634bc67` reconfirmed the same first-token red box (`[11,11,11,315,315]` / `",,, of of"` vs `[34,2735,35,12,7854]` / `"CMLD-204"`), so do not treat 2048 context as supported for this row
...
- **Blocked now:** the attempted second bounded 1024-context pack and third bounded 2048-context pack both timed out in Camelid `/v1/chat/completions` before producing parity reports; do not treat 1024/2048 context as supported for this row
- **Runtime follow-up:** a local code-only chunked prefill/Q8 read-reuse slice now exists behind `BACKENDINFERENCE_PREFILL_CHUNK_TOKENS` and is covered by local fmt/clippy/test/release-build gates; it is not a support-status change without row-specific Ubuntu parity/timing artifacts
- **Missing gates:** performance/memory architecture work for KV/cache and attention behavior, Q8 matmul/output-projection hot paths, dense/f32 materialization avoidance, measured chunked/streamed long-context prefill, per-layer timing/RSS-peak instrumentation, stronger performance evidence, portable packaging evidence, and broader chat-template coverage
...
For **Llama 3.2 1B** specifically, the committed carry-forward row bundle at `qa/evidence-bundles/four-row-public-20260503T024327Z/llama32_1b_instruct_q8_0.bundle.json`, the current-head row manifest at `qa/evidence-bundles/four-row-current-head-20260503T061958Z-head-34b954498a03/llama32_1b_instruct_q8_0/manifest.json`, the four-row context bundle at `qa/evidence-bundles/four-row-context-512-20260505T051510Z-head-b403884/manifest.json`, the second bounded 1024-context bundle at `qa/evidence-bundles/llama32-1b-context-1024-20260505T081001Z-head-156ded6fc76b/manifest.json`, the compact template-shapes bundle at `qa/evidence-bundles/llama32-1b-3b-chat-template-shapes-20260505T060036Z-head-e9f28572e090/manifest.json`, and the unique-chat perf/RSS bundle at `qa/evidence-bundles/llama32-1b-3b-unique-chat-perf-rss-20260505T061644Z-head-e9f28572e090/manifest.json` are the durable citation anchors for the exact-row smoke, first bounded 512-context boundary, second bounded 1024-context boundary, bounded compact template-shape boundary, and bounded memory/perf envelope. The attempted 2048-context pack is documented only as a blocker at `qa/validation-notes/2026-05-05-llama32-1b-context-2048-blocker.md`, with a clean current-main blocker reconfirmation at `qa/validation-notes/2026-05-05-llama32-1b-q8-2048-current-main-watchdog.md`. Model-native/larger context, arbitrary/Jinja template behavior, production throughput, and portability reruns remain exact-row follow-up work.
...
| 1k / 2k tokens | Row-specific only | Llama 3.2 1B/3B have checked 1024-context passes and Llama 3.2 3B has a checked 2048-context pass. Llama 3 8B 1024/2048 attempts are blocked by backend timeout, so 8B long-context promotion is paused pending performance/memory architecture work and fresh PASS artifacts. |
...
- **Phase 11 — Llama 3 / Backend + QA:** keep broad Llama 3 below support until each concrete target has the right artifact-backed evidence. For Llama 3.2 1B Q8_0, exact-row short-chat smoke support is now strengthened by a passing three-prompt known-good-baseline pack in the downloaded-model matrix, the first bounded 512-context pack, and the second bounded 1024-context pack. For Llama 3.2 3B Q8_0, exact-row smoke support is now strengthened by compact parity, API, frontend, smoke-pack evidence, the post-Q8-dot clean broader three-prompt 50-token parity rerun, the first bounded 512-context pack, the second bounded 1024-context pack, and the third bounded 2048-context pack; model-native/larger-context, performance/portability, and broader chat-template evidence remain the next requirements before widening the row. For Llama 3 8B, compact prompt-token, 1-token, 5-token, bounded 50-token parity, the three-prompt 50-token broader pass, basic API/frontend smoke, the first bounded 512-context pack, and bounded memory evidence now support the exact row; the attempted 1024-context diagnostic and 2048-context pack are explicitly blocked by Camelid backend non-completion within the 900-second parity harness timeout. Under Tim's rule, pause 8B long-context promotion and shift Backend/Performance work to KV/cache and attention memory behavior, Q8 matmul/output-projection hot paths, dense/f32 materialization avoidance, chunked/streamed long-context prefill, and per-layer timing/RSS-peak instrumentation, preserving TinyLlama/1B/3B short-context guardrails and serial/block-aligned determinism until optimized kernels have separate zero-delta evidence.
...
Current evidence handoff: Llama 3 8B now has compact-header `hello` parity against llama.cpp for prompt tokens plus deterministic 1-token, 5-token, and bounded 50-token generation, basic API/frontend smoke, bounded memory evidence, a passing three-prompt 50-token Ubuntu parity run, the first bounded 512-context pack, and a retained-block lazy-Q8 hot-path cost bundle for representative FFN/output tensors for the exact same row. The required 1024-context diagnostic and attempted 2048-context pack both timed out in Camelid before parity reports, so the next status-changing work is not another long-context promotion attempt; it is performance/memory architecture plus instrumentation, followed only later by fresh PASS artifacts before any 1k/2k promotion. Current `bench-q8-blocks` memory fields and representative attention Q/K/V/output, FFN, and output-projection shape evidence should travel with that handoff: retained Q8 payload, avoided `f32` materialization, bounded dot input, optional all-row output vector, and the guarded swapped rank-2 logical row/column interpretation distinguish safe lazy-execution scratch/output buffers from unsafe eager `f32` weight decoding. The deterministic-parallelism metadata should travel too: current Q8 block rows are serial-only, no parallel Q8 kernel is enabled by default, and any future serial-vs-parallel comparison must target zero delta with a `1e-7` fail threshold before it can affect support claims. The independent reference token dumps for the existing Llama 3 fixture prompts are complete for tokenizer parity evidence, but they do not unlock broader 8B generation support by themselves.
