...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TinyLlama 1.1B Chat Q8_0 | Supported current gate | Documented PASS for the TinyLlama gate, including 5-prompt/50-token parity. Cited artifacts are under gitignored `target/` and are not present in this public worktree checkout. | Fresh reopened-lane exact-row API smoke is captured in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`. | Fresh reopened-lane frontend smoke is captured in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`. | Documented as measured for the current gate; exact support-grade current-head perf artifact is not present in this checkout. | Public docs/API/frontend agree this is supported. | Current public evidence through `595b3e3` preserves reopened-lane API/WebUI smoke evidence for this row. | Mostly evidence-packaging/recency for full-support normalization: preserve current-head parity/perf artifacts in a durable location before using it as part of the four-row full-support release. |
| Llama 3.2 1B Instruct Q8_0 | Supported exact-row smoke, not broad/full support | Documented PASS for compact parity and the broader prompt pack. Cited artifacts are under gitignored `target/` and are not present in this checkout. | Fresh reopened-lane exact-row API smoke is captured in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`. | Fresh reopened-lane frontend smoke unlocks supported WebUI chat for this exact row in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`. | Only bounded smoke runs are claimed. Stronger memory/perf, longer-context, and portability evidence are still missing. | Public docs/API/frontend agree on **short local-chat smoke** only. | Current public evidence through `595b3e3` preserves 1B exact-row smoke support and the reopened-lane API/WebUI refresh. | Missing full-support expansion evidence: longer context, broader chat-template behavior, stronger memory/perf, portability, and durable current-head artifacts. |
| Llama 3.2 3B Instruct Q8_0 | Supported exact-row smoke, not broad/full support | Documented PASS for compact prompt-token/1-token/5-token/50-token parity and the post-Q8-dot broader 3-prompt/50-token pack. Cited artifacts are under gitignored `target/` and are not present in this checkout. | Fresh reopened-lane exact-row API smoke is captured in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`. | Fresh reopened-lane frontend smoke unlocks supported WebUI chat for this exact row in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`. | Only bounded smoke/perf follow-up is claimed; stronger memory/perf envelope, longer contexts, and portability are missing. | Public docs/API/frontend agree on **short local-chat smoke** only; this is intentionally not a broad Llama-family claim. | Current public evidence through `595b3e3` preserves 3B exact-row smoke support and the reopened-lane API/WebUI refresh. | Missing full-support expansion evidence: longer context, broader chat-template acceptance, stronger memory/perf, portability, and durable current-head artifacts. |
| Llama 3 8B Instruct Q8_0 | Supported exact-row smoke, not broad/full support | Documented PASS for compact prompt-token/1-token/5-token/bounded-50-token parity, the three-prompt 50-token Ubuntu pack, the first bounded 512-context pack, and the bounded compact chat-template-shapes pack. | Fresh reopened-lane exact-row API smoke is captured in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`; a patched-tree current-head API/WebUI/RSS timing smoke for `/v1/completions` diagnostics passed at `qa/evidence-bundles/llama3-8b-api-webui-rss-20260505T014408Z-head-8cef7af4d6c6/manifest.json`. | Fresh reopened-lane frontend smoke unlocks supported WebUI chat for this exact row in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`; the patched-tree 8B smoke also generated `Hello` with WebUI chat enabled. | Bounded compact/backend memory evidence exists, the first 512-context pack passed with timed-process max RSS `17262740 KiB`, and the patched-tree API/WebUI smoke sampled backend RSS `6308 -> 286216 KiB`; support-grade performance envelope, portability evidence, and broader context buckets are still missing. | Public docs/API/frontend now agree on **short local-chat/parity smoke** plus one bounded broader 50-token pack, one bounded 512-context pack, one compact chat-template-shapes pack, and one patched-tree completion-diagnostics API timing slice only; this remains intentionally narrower than broad Llama-family support. | Current local evidence through `8cef7af` plus the API diagnostics patch preserves exact-row 8B API/WebUI/timing evidence; push plus a clean-head rerun are still needed before using it as final release citation. | Missing full-support expansion evidence: broader context, broader chat-template acceptance, stronger memory/perf, portability, and durable clean-head artifacts. Current note: the earlier first bounded longer-context timeout and compact template-shape gap are cleared for checked packs only; see `qa/validation-notes/2026-05-05-8b-broader-50tok.md`, `qa/validation-notes/2026-05-04-8b-context-512-rerun.md`, `qa/validation-notes/2026-05-05-8b-chat-template-shapes.md`, and `qa/validation-notes/2026-05-05-8b-api-webui-rss.md`. |
...
- Current sanitized carry-forward bundle/checksum roots are `qa/evidence-bundles/four-row-public-20260503T024327Z/`, `qa/evidence-bundles/four-row-perf-portability-public-20260503T025639Z/`, `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/`, `qa/evidence-bundles/llama3-8b-broader-50tok-20260505T005031Z-head-d13541ad8d7e/`, `qa/evidence-bundles/llama3-8b-context-512-20260504T234625Z-head-58acf592345c/`, `qa/evidence-bundles/llama3-8b-chat-template-shapes-20260505T003821Z-head-d13541ad8d7e/`, and `qa/evidence-bundles/llama3-8b-api-webui-rss-20260505T014408Z-head-8cef7af4d6c6/`; they improve reviewability but do **not** replace the remaining full-support parity/perf/portability reruns.
...
- Fresh `/api/models/load`, `/v1/models`, `/v1/completions`, `/v1/chat/completions` artifacts saved for the exact 8B row. Current note: the patched-tree completion-diagnostics smoke at `qa/evidence-bundles/llama3-8b-api-webui-rss-20260505T014408Z-head-8cef7af4d6c6/manifest.json` validates the API timing-summary path but still needs a clean-head committed rerun if it is used as final release evidence.
...
- Current public main base: `8cef7af4d6c6198210681257f2b7b111d5801ff4 Make full-support scaffold help side-effect free`; the exact-row smoke guardrails and reopened-lane evidence remain preserved in that landed state. The new completion-diagnostics API evidence was validated as a local patch on top of this base and still needs push plus a clean-head rerun if used as final release evidence.
- Local `target/` has fresh 2026-05-05 normalized scaffold/audit outputs including `target/full-support-20260505T013036Z-head-595b3e34953e/manifest.json` and `target/evidence-bundle-privacy-audit-20260505-watchdog.json`, which reports `finding_count: 0` for committed `qa/evidence-bundles`. These are local watchdog artifacts only because `/target/` is gitignored.
...
- Reopened-lane 8B broader 50-token, 512-context, compact chat-template-shapes, and patched-tree API/WebUI/RSS timing summaries now live under their dedicated `qa/evidence-bundles/llama3-8b-*` directories. They close only those exact checked 8B slices.
