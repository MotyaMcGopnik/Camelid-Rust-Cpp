...
| Llama 3 8B Instruct Q8_0 | Supported exact-row smoke, not broad/full support | Documented PASS for compact prompt-token/1-token/5-token/bounded-50-token parity, the three-prompt 50-token Ubuntu pack, the first bounded 512-context pack, and the bounded compact chat-template-shapes pack. | Fresh reopened-lane exact-row API smoke is captured in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`. | Fresh reopened-lane frontend smoke unlocks supported WebUI chat for this exact row in `qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`. | Bounded compact/backend memory evidence exists, and the first 512-context pack passed with timed-process max RSS `17262740 KiB`; support-grade performance envelope, portability evidence, and broader context buckets are still missing. | Public docs/API/frontend now agree on **short local-chat/parity smoke** plus one bounded broader 50-token pack, one bounded 512-context pack, and one compact chat-template-shapes pack only; this remains intentionally narrower than broad Llama-family support. | Current public evidence through `595b3e3` preserves exact-row 8B smoke/API/WebUI evidence plus the broader 50-token and compact chat-template-shapes bundle summaries beside the earlier 512-context evidence. | Missing full-support expansion evidence: broader context, broader chat-template acceptance, stronger memory/perf, portability, and durable current-head artifacts. Current note: the earlier first bounded longer-context timeout and compact template-shape gap are cleared for checked packs only; see `qa/validation-notes/2026-05-05-8b-broader-50tok.md`, `qa/validation-notes/2026-05-04-8b-context-512-rerun.md`, and `qa/validation-notes/2026-05-05-8b-chat-template-shapes.md`. |
...
- Current public main base: `595b3e34953e52ab4ff8637e2109844c38071e64 Normalize bounded 8B support evidence`; the exact-row smoke guardrails and reopened-lane evidence remain preserved in that landed state.
...
- Reopened-lane 8B broader 50-token, 512-context, and compact chat-template-shapes summaries now live under their dedicated `qa/evidence-bundles/llama3-8b-*` directories. They close only those exact checked 8B packs.
