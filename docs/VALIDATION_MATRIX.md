...
- Current-head language requires a fresh canonical PASS after later runtime/source commits before a bounded bucket can be called current-head. The Llama 3 8B 1024/2048 buckets have that published source/runtime-head PASS at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T041451Z-head-8e26be0a73c0/manifest.json`; broad/full support for every non-TinyLlama row still requires model-native/larger context beyond checked packs, arbitrary-template evidence, throughput, portability, and durable normalized current-head bundles.
...
| Frontend-only copy/layout | `cd frontend && npm ci && npm run build` | `npm run smoke:streaming` for stream-parser/chat transport changes; `npm run smoke` or `npm run smoke:tiny` when chat/model-load/readiness surfaces change | Do not loosen readiness gates or support wording without matching evidence/docs updates. |
