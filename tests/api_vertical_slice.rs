...
async fn capabilities_report_support_contract_and_planned_lanes() {
    ...
    assert!(
        q8_notes.contains("exact Llama 3 8B Q8_0 row has checked bounded 512-context support only")
    );
    assert!(q8_notes.contains(
    ...
    assert_eq!(
        ...
        "short_api_webui_smoke_with_broader_50_token_plus_checked_512_context_pack_1024_2048_red"
    );
    ...
    assert_eq!(llama3["latest_checked_output"], "CMLD-512-CHECK");
    let llama3_evidence = llama3["evidence"].as_str().unwrap();
    assert!(llama3_evidence.contains("checked bounded 512-context pack"));
    assert!(llama3_evidence.contains("8B 1024/2048 remain red"));
    ...
    assert!(llama3_next_step.contains("keeping 8B 1024/2048 red"));
