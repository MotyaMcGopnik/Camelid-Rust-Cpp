...
async fn capabilities_report_support_contract_and_planned_lanes() {
    ...
    assert!(q8_notes.contains(
        "exact Llama 3 8B Q8_0 row is promoted only through its checked bounded 512-context pack"
    ));
    assert!(q8_notes.contains("8B 1024/2048 buckets remain red/not promoted"));
    ...
    assert!(llama_bpe_notes.contains("exact Llama 3 8B Instruct Q8_0 has row-specific smoke support with a checked bounded 512-context pack only"));
    assert!(llama_bpe_notes.contains("8B 1024/2048 buckets remain red/not promoted"));
    ...
    assert_eq!(llama3["bounded_context_1024_pack_id"], "not_promoted");
    assert_eq!(llama3["bounded_context_1024_window"], 1024);
    assert_eq!(
        llama3["bounded_context_2048_pack"],
        "blocked_backend_timeout_900s"
    );
    assert_eq!(llama3["bounded_context_2048_pack_id"], "not_promoted");
    ...
    assert!(llama3_evidence.contains("1024/2048 buckets remain red/not promoted"));
    ...
    assert!(llama3_next_step.contains("fresh 8B 1024/2048 PASS evidence"));
