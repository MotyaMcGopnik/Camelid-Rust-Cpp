...
async fn capabilities_report_support_contract_and_planned_lanes() {
    ...
    assert!(q8_notes.contains(
        "exact Llama 3.2 1B/3B Q8_0 rows have checked bounded 512/1024/2048-context packs, while exact Llama 3 8B Q8_0 is checked only through the bounded 512-context pack"
    ));
    ...
    assert!(llama_bpe_notes.contains(
        "exact Llama 3.2 1B/3B Instruct Q8_0 have row-specific smoke support with checked bounded 512/1024/2048-context packs; exact Llama 3 8B Instruct Q8_0 has row-specific smoke support with checked bounded 512-context only"
    ));
    assert!(llama_bpe_notes.contains("8B broader 50-token"));
    ...
    assert!(
        llama3_evidence.contains("1024/2048 remain red pending fresh current-head PASS artifacts")
    );
