...
async fn capabilities_report_support_contract_and_planned_lanes() {
    ...
    assert!(q8_notes.contains(
        "exact Llama 3.2 1B/3B/8B Q8_0 rows have checked bounded 512/1024/2048-context packs where row-specific PASS artifacts exist"
    ));
    ...
    assert!(llama_bpe_notes.contains(
        "exact Llama 3.2 1B/3B/8B Instruct Q8_0 have row-specific smoke support with checked bounded 512/1024/2048-context packs where PASS artifacts exist"
    ));
    ...
    assert!(llama3_evidence.contains("q8_file_reads counters"));
