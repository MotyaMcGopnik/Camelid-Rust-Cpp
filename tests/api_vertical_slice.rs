...
async fn capabilities_report_support_contract_and_planned_lanes() {
    ...
    assert_eq!(llama3["latest_checked_output"], "bounded_512_pack_pass");
    let llama3_evidence = llama3["evidence"].as_str().unwrap();
    assert!(llama3_evidence.contains("checked 512-context pack"));
    assert!(llama3_evidence.contains("1024/2048-context buckets remain unpromoted"));
    ...
    assert!(llama3_next_step.contains("fresh 1024/2048 PASS artifacts"));
