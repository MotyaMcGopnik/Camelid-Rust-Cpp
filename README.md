...
- **New Llama 3-family evidence, not a support expansion:** Camelid now has stronger exact-row evidence for tracked Llama 3-family lanes, but no Llama 3-family lane is supported today.
  ...
  - **3B acceptance target / first-token evidence only:** the exact tracked Llama 3.2 3B Instruct Q8_0 GGUF now loads successfully through `/api/models/load` with low backend RSS after streaming metadata parsing plus lazy/file-backed Q8 handling, and one healthy Ubuntu backend-only first-token success artifact now exists. This is exact-row blocker reduction, not a support promotion, and the row remains blocked on repeatability, parity, API, and WebUI evidence.
  - **8B groundwork / first-token evidence only:** Llama 3 8B Instruct Q8_0 now has one healthy Ubuntu backend-only first-token artifact on the exact tracked Q8_0 GGUF, on top of the earlier tokenizer/config/Q8 groundwork and lazy/file-backed execution work.
- **Public support contract stays unchanged:** 1B remains evidence-only, 3B remains an acceptance target with first-token evidence only, 8B remains groundwork-only with one backend-only first-token artifact, and no Llama 3-family row is a supported generation lane today.
...
| Llama 3.2 3B Instruct Q8_0 | Acceptance target / first-token evidence only | The exact tracked GGUF is present locally; `/api/models/load` now succeeds with low backend RSS after streaming metadata parsing; file-backed lazy-Q8 materially reduced the earlier eager dense-load spike; one healthy Ubuntu backend-only `/v1/completions` first-token success artifact now exists for `hello`. | Not a supported row. No 3B repeatability, prompt-token parity, short-generation parity, API readiness, or WebUI readiness claim until bounded exact-row evidence exists. |
| Llama 3 8B Instruct Q8_0 | Groundwork only / first-token evidence only | Metadata, tokenizer/config fixtures, independent tokenizer references, lazy/file-backed Q8 execution groundwork, and one healthy Ubuntu backend-only `/v1/completions` first-token artifact for `hello` now exist. | No supported generation, prompt-token parity, repeat bounded success, frontend readiness, or portable-packaging claim until exact-row follow-up evidence exists. |
...
- **Llama 3.2 3B Instruct Q8_0:** the exact tracked GGUF is present locally, the row now loads successfully through `/api/models/load` with low backend RSS after streaming metadata parsing, and one healthy Ubuntu backend-only first-token success artifact now exists. Recent file-backed lazy-Q8 recovery materially reduced the old eager dense-load spike, but the row is still not supported and remains blocked on repeat bounded success plus prompt-token parity, short-generation parity, API evidence, and WebUI evidence. See [`QA_LLAMA32_3B_Q8_ACCEPTANCE.md`](QA_LLAMA32_3B_Q8_ACCEPTANCE.md) and [`STATUS.md`](STATUS.md) for the exact blocker state.
- **Llama 3 8B Instruct Q8_0:** metadata/config handling, GQA/RoPE guardrails, tokenizer/template fixtures, independent llama.cpp tokenizer references, and lazy/file-backed Q8 execution are now far enough along to support one healthy Ubuntu backend-only first-token artifact for `hello` on the exact tracked Q8_0 GGUF. That is encouraging groundwork, not a support promotion: Camelid still does not claim 8B prompt-token parity, short-generation parity, API/frontend readiness, or repeat bounded success for this row yet.
