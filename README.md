...
- **The context ladder is explicit.** The four supported Llama/TinyLlama rows have checked bounded 512-context evidence; Llama 3.2 1B/3B and Llama 3 8B also have checked 1024 and 2048 packs. Mixtral remains fail-closed at the bounded one-token backend MoE runtime support boundary; 5-token/API/WebUI/RSS promotion-candidate artifacts are documented as superseded and do not promote frontend support, longer context, or broad Mixtral readiness.
...
| Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf | Bounded backend MoE runtime probe: one-token smoke with selected-weight renormalized top-k routing and lazy/file-backed rank-3 Q8 experts | One-token support boundary only; later promotion-candidate artifacts are superseded by 50-token divergence and a long-continuation hang | Backend runtime boundary only; superseded API/WebUI/RSS artifacts do not promote frontend support, long-context, or broad support |
