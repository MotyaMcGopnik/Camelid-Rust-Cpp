...
- **The context ladder is explicit.** The four supported Llama/TinyLlama rows have checked bounded 512-context evidence; Llama 3.2 1B also has checked 1024/2048/4096/8192 packs; Llama 3.2 3B and Llama 3 8B also have checked 1024 and 2048 packs. Mixtral has no promoted context bucket; only bounded one-token backend MoE runtime evidence is retained.
...
- **Supported-row hardening:** preserve TinyLlama as the full current gate, move the verified Llama rows toward the same normalized bar, and keep support wording tied to green evidence. The remaining gaps are broader context, arbitrary/Jinja template coverage, production-throughput evidence, portability, and repeated current-head bundles.
  - Mixtral continuation hardening is active validation only: bounded one-token backend MoE runtime evidence exists, but later-generation parity diverges and a longer-continuation backend HTTP hang keeps API/WebUI/frontend readiness blocked.
...
| Llama 3.2 1B Instruct Q8_0 | **Verified support (bounded)** | Load, completions, chat completions, WebUI validation, compact/broader parity, bounded template-shape checks, bounded unique-chat perf/RSS, and checked 512/1024/2048/4096/8192-context packs. | The 8192 pass is exact-row/current-head only via `qa/evidence-bundles/llama32-1b-context-8192-current-head-20260513T183501Z-head-aaf9207d1669/manifest.json`; it is not model-native/larger-context beyond checked packs, arbitrary-template, production-throughput, portability, or neighboring-row support. |
...
The maintainer matrix now includes four exact Q8_0 supported rows with checked row-specific evidence. Mistral remains an active validation row, and Mixtral remains partial backend runtime evidence only, not a supported row. These are bounded checks, not universal model claims.

| Exact row | Latest checked bucket | Result | Output checked |
| --- | --- | --- | --- |
| TinyLlama 1.1B Chat Q8_0 | direct chat validation | PASS | `Certainly! Here` |
| Llama 3.2 1B Instruct Q8_0 | 8192-context bounded recall pack | PASS on current `main` | `CMLD-819` |
...
- Support language currently means only the exact supported rows above; Mistral has no support claim yet and may only be discussed as the exact active validation row above. The Mixtral row may only be described as bounded one-token backend MoE runtime evidence with later-generation/API/WebUI/frontend readiness blocked.
