...
- **Real Llama 3-family breakthrough, not a support expansion:** Camelid now has materially better Llama 3-family mechanics and evidence than it did before.
  ...
  - **3B blocker-seam breakthrough:** the exact tracked Llama 3.2 3B Instruct Q8_0 GGUF now loads through `/api/models/load` with low backend RSS after streaming metadata parsing plus lazy/file-backed Q8 handling, and one healthy Ubuntu backend-only first-token success artifact now exists. This is real blocker-seam progress, not a support promotion.
  - **8B first-token groundwork breakthrough:** Llama 3 8B Instruct Q8_0 now has one healthy Ubuntu backend-only first-token artifact on the exact tracked Q8_0 GGUF, on top of the earlier tokenizer/config/Q8 groundwork and lazy/file-backed execution work.
