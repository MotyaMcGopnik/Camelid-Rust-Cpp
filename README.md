...
> **Current public posture:** Camelid achieves 1:1 parity with llama.cpp for five exact GGUF rows within a bounded published validation envelope: TinyLlama at the current validated gate; Llama 3.2 1B/3B Q8_0 and Llama 3 8B Q8_0 through checked bounded 512/1024/2048-context packs; and Mistral 7B Instruct v0.3 Q8_0 through tokenizer/template parity, API chat smoke, and checked bounded 512/1024/2048-context packs. The fresh Ubuntu Mistral revalidation bundle at `qa/evidence-bundles/mistral-7b-v0.3-q8-context-512-1024-2048-ubuntu-20260508T203513Z-head-86ad5390d265/manifest.json` is exact-row bounded-pack evidence for pushed head `86ad5390d265`; the 8B bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T202751Z-head-86ad5390d265/manifest.json` remains exact-row bounded-pack evidence for its cited runtime/API/frontend head. These are exact-row bounded-pack claims only; wider model-native context, production throughput, portability, arbitrary templates, and broad-family support remain outside the claim.
...
- **Five exact Q8_0 rows are public and evidence-backed.** TinyLlama remains the full current gate; Llama 3.2 1B, Llama 3.2 3B, Llama 3 8B, and Mistral 7B Instruct v0.3 have checked bounded support for their exact Q8_0 rows where row-specific PASS bundles are cited.
...
- **The context ladder is explicit.** TinyLlama stays at the current gate with bounded 512-context coverage; Llama 3.2 1B/3B/8B and Mistral 7B Instruct v0.3 have checked bounded 512/1024/2048-context evidence where row-specific PASS bundles are cited. These checked packs do not imply model-native/larger context, arbitrary-template, production-throughput, portability, or broader-family support.
...
- **Supported-row hardening:** preserve TinyLlama as the current full gate and move the Llama and Mistral exact-row verified lanes toward the same normalized bar without overstating them. The next promotable evidence remains model-native/larger-context behavior beyond checked packs, arbitrary/Jinja template coverage, production-throughput evidence, portability, and repeated current-head bundles. CI reliability is non-negotiable: no public support wording should move if the gate is red.
- **Active next-model bring-up set:** Mistral 7B Instruct v0.3 is now the first next-family exact row with bounded smoke support. Camelid is still publicly working on **Mixtral 8x7B Instruct**, **Qwen 2.5 7B Instruct**, and **Gemma 2 9B Instruct** as planned exact-row candidates.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — exact-row bounded smoke support: tokenizer/template parity, chat-completion smoke, and Ubuntu-revalidated 512/1024/2048 packs pass. No Mistral-family, neighboring-row, 50-token, arbitrary-template, production-throughput, portability, or full-support claim is implied.
...
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | **Verified support (bounded)** | Tokenizer/template parity, first-token parity, `/v1/chat/completions` smoke, and Ubuntu-revalidated 512/1024/2048-context packs. A live Ubuntu chat smoke answered ` Hello there! How can I assist you` for an 8-token request. | Exact-row bounded smoke only: no Mistral-family support, neighboring variants, 50-token parity, arbitrary templates, model-native/larger context, production throughput, portability, or full support. |
...
The maintainer matrix now includes five exact Q8_0 supported rows with checked row-specific evidence. These are bounded checks, not universal model claims.
...
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | Ubuntu 2048-context bounded recall pack + live chat smoke | PASS | ` The repeat marker is "` / ` Hello there! How can I assist you` |
