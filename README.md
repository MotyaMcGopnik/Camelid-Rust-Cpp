...
> **Current public posture:** Camelid achieves 1:1 parity with llama.cpp for four exact GGUF rows within a bounded published validation envelope: TinyLlama at the current validated gate, Llama 3.2 1B/3B Q8_0 through checked bounded 2048-context packs, and Llama 3 8B Q8_0 through the checked bounded 512-context pack on current `main`; the older 1024/2048 PASS bundle is historical for head `ab8e465b50c3` until a fresh canonical current-head PASS lands. These are exact-row bounded-pack claims only; wider model-native context, production throughput, portability, arbitrary templates, and broad-family support remain outside the claim.
...
- **Four exact Q8_0 rows are public and evidence-backed.** TinyLlama remains the full current gate; Llama 3.2 1B and Llama 3.2 3B have checked bounded 512/1024/2048-context support, while Llama 3 8B is exposed on current `main` through the checked bounded 512-context pack only until 1024/2048 are rerun on the latest runtime head.
...
- **The context ladder is explicit.** TinyLlama stays at the current gate with bounded 512-context coverage; Llama 3.2 1B/3B have checked bounded 512/1024/2048-context evidence, and Llama 3 8B keeps 1024/2048 red on current `main` until fresh PASS artifacts and synchronized docs/API/frontend alignment exist. These checked packs do not imply model-native/larger context, arbitrary-template, production-throughput, portability, or broader-family support.
...
| Llama 3 8B Instruct Q8_0 | **Verified support (bounded)** | Load, completions, chat completions, WebUI validation, compact parity, three-prompt 50-token parity, the checked 512-context pack, compact chat-template-shapes pack, bounded memory evidence, and lazy-Q8 hot-path measurements. The 1024/2048 PASS bundle remains cited only for its older source head until rerun on current `main`. | 1024/2048 are not current-head green after later Q8 runtime/source commits; do not claim them until fresh canonical PASS artifacts and support-surface alignment exist. No model-native/larger context, production throughput, arbitrary templates, portability, neighboring-row, or broad 8B/Llama support is implied. |
...
| Llama 3 8B Instruct Q8_0 | 512-context bounded recall pack; 1024/2048 require rerun on current head | PASS at 512 / red for current-head 1024/2048 | historical `CMLD-204` at `ab8e465b50c3` only |
...
- The next major 8B support gaps are model-native/larger context beyond checked packs, arbitrary templates, production throughput, portability, and repeated durability evidence.
