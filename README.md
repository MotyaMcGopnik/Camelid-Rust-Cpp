...
| Llama 3.2 3B Instruct Q8_0 | Supported exact-row smoke | Public carry-forward smoke bundle, current-head row manifest, post-Q8-dot broader-pack handoff, four-row 512-context pack, bounded compact chat-template-shapes pack, and bounded unique-chat perf/RSS envelope anchor this row only. | Broader/larger context beyond the first bounded 512 pack, production throughput/portability evidence, arbitrary/Jinja template behavior beyond the checked compact pack, and no implied support for neighboring rows. |
...
- `qa/evidence-bundles/8b-checkmark-current-head-20260505T052647Z-head-864e07b51f36/manifest.json` plus `SHA256SUMS` is the latest public-main exact 8B checkmark refresh: API/WebUI/RSS smoke passed at head `864e07b51f36` with max sampled backend RSS `286056 KiB`, preserving exact-row smoke only.
