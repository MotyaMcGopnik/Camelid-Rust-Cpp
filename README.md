...
| Gemma 4 two-Mac sharding | Supported (two-Mac distributed) | `gemma4-master`/`gemma4-worker` run one Gemma 4 row split across two machines (distributed layer sharding over TCP — NOT shared memory) with a versioned handshake and per-packet checksums; distributed greedy output is asserted token-identical to single-node and to the llama.cpp oracle (`tests/gemma4_distributed_parity.rs`). This is the supported serve lane for the two rows too large for a single 16 GB host: **Gemma 4 12B-It Q8_0** (12.67 GB) and **Gemma 4 26B-A4B-It QAT Q4_0** (13.4 GB, 128-expert MoE). With `CAMELID_GEMMA4_SERVE=1` plus `CAMELID_GEMMA4_WORKER`/`CAMELID_GEMMA4_SPLIT`, `/v1/chat/completions` and `/v1/completions` route through a persistent master shard with per-request worker sessions (wire protocol v1). Both rows are proven on two 16 GB M4 Mac minis: distributed output token-identical to single-node, full basic_v1 parity vs the pinned reference (12B 5/5 distributed-vs-single-node; 26B A4B 2/5 full-budget token-identical + 3/5 probe-verified knife-edge frontiers), and the distributed serve/WebUI promotion smoke green for both (chat, raw completion, SSE; evidence bundles in qa/evidence-bundles). See [`docs/gemma4-two-mac-cluster.md`](docs/gemma4-two-mac-cluster.md). |
