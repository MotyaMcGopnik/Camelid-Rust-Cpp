...
> **Current public posture:** Camelid achieves 1:1 parity with llama.cpp for four exact GGUF rows within a bounded published validation envelope: TinyLlama at the current validated gate, Llama 3.2 1B/3B Q8_0 through checked bounded 512/1024/2048-context packs, and Llama 3 8B Q8_0 through checked bounded 512/1024/2048-context packs. The fresh 8B 1024/2048 bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T173841Z-head-aa315788ce2e/manifest.json` is exact-row bounded-pack evidence for runtime/API/frontend head `aa315788ce2e`. These are exact-row bounded-pack claims only; wider model-native context, production throughput, portability, arbitrary templates, and broad-family support remain outside the claim.
...
- **Four exact Q8_0 rows are public and evidence-backed.** TinyLlama remains the full current gate; Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B have checked bounded 512/1024/2048-context support for their exact Q8_0 rows where row-specific PASS bundles are cited.
...
- **The context ladder is explicit.** TinyLlama stays at the current gate with bounded 512-context coverage; Llama 3.2 1B/3B/8B have checked bounded 512/1024/2048-context evidence where row-specific PASS bundles are cited. These checked packs do not imply model-native/larger context, arbitrary-template, production-throughput, portability, or broader-family support.
...
- **Next-model bring-up set:** Camelid has promoted the first Mistral exact row to a narrow first-token smoke lane while keeping the remaining next-family rows planned.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — exact-row first-token smoke support only: checked tokenizer/template references plus 1-token reference parity pass. Broader/full Mistral support still needs 50-token parity, API/WebUI smoke bundles, RSS/timing, 1024/2048 checks, portability, and repeated current-head evidence.
...
| Llama 3 8B Instruct Q8_0 | **Verified support (bounded)** | Load, completions, chat completions, WebUI validation, compact parity, three-prompt 50-token parity, checked 512/1024/2048-context packs, compact chat-template-shapes pack, bounded memory evidence, structured RSS/Q8 file-read counters, and lazy-Q8 hot-path measurements. | The 1024/2048 pass is exact-row bounded-pack evidence for runtime/API/frontend head `aa315788ce2e`; no model-native/larger context, production throughput, arbitrary templates, portability, neighboring-row, or broad 8B/Llama support is implied. |
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | **First-token smoke support (exact row)** | Checked tokenizer/template prompt references and 1-token generated-token/text parity (`[23325]` / ` Hello`) in `qa/evidence-bundles/mistral-7b-v0.3-q8-1tok-parity-20260508T1847Z-head-fa7efc086c0e/`. | Exact-row first-token only: no broader Mistral-family support, neighboring Mistral row, 50-token output, 1024/2048 context, production-throughput, portability, or full support yet. |
...
- The next major 8B support gaps are model-native/larger context beyond checked packs, arbitrary templates, production throughput, portability, and repeated durability evidence beyond the fresh bounded 1024/2048 PASS at runtime/API/frontend head `aa315788ce2e`.
