...
> **Current public posture:** Camelid achieves 1:1 parity with llama.cpp for four exact GGUF rows within a bounded published validation envelope: TinyLlama at the current validated gate, Llama 3.2 1B/3B Q8_0 through checked bounded 512/1024/2048-context packs, and Llama 3 8B Q8_0 through the checked bounded 512-context pack only. The 8B 1024/2048 bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T091025Z-head-844e8d4709d1/manifest.json` is source-head evidence for `844e8d4709d1`; after later source/runtime changes it is historical until a fresh current-head canonical PASS lands. These are exact-row bounded-pack claims only; wider model-native context, production throughput, portability, arbitrary templates, and broad-family support remain outside the claim.
...
- **Four exact Q8_0 rows are public and evidence-backed.** TinyLlama remains the full current gate; Llama 3.2 1B and Llama 3.2 3B have checked bounded 512/1024/2048-context support for their exact Q8_0 rows, while Llama 3 8B remains promoted only through the checked bounded 512-context pack on current `main` until a fresh post-change 1024/2048 PASS exists.
...
| Llama 3 8B Instruct Q8_0 | 512-context bounded recall pack | PASS | `reference_match_recorded` |
...
- The next major 8B support gaps are a fresh current-head 1024/2048 canonical PASS after the latest runtime/source changes, model-native/larger context beyond checked packs, arbitrary templates, production throughput, portability, and repeated durability evidence.
