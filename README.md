...
> **Current public posture:** Camelid achieves 1:1 parity with llama.cpp only for the supported exact GGUF rows with published validation evidence: TinyLlama at the current validated gate; Llama 3.2 1B Q8_0 with end-to-end support at checked 512/1024/2048/4096/8192-context packs; Llama 3.2 3B Q8_0 at checked 512/1024/2048-context packs; and Llama 3 8B Q8_0 through exact-row smoke plus checked 512/1024/2048-context packs on current `main`. `Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` has one-token backend MoE runtime evidence, but Gate 9A later-generation evidence diverges and a longer-continuation backend HTTP hang remains unresolved, so Mixtral API/WebUI/frontend readiness and broad Mixtral support are not claimed. `Mistral-7B-Instruct-v0.3.Q8_0.gguf` remains an active exact-row bring-up lane, not a supported row. These are exact-row claims only; wider model-native context beyond the checked packs, production throughput, portability, arbitrary templates, neighboring rows, and broad-family behavior remain outside the claim.
...
- **Four exact Q8_0 rows are public and evidence-backed.** TinyLlama remains the full current gate; Llama 3.2 1B has end-to-end support at checked 512/1024/2048/4096/8192 contexts; Llama 3.2 3B and Llama 3 8B are supported at their listed checked contexts. Mixtral has one-token backend MoE runtime evidence only and remains blocked by later-generation divergence; Mistral 7B Instruct v0.3 remains fail-closed as an active bring-up lane until its current-head promotion checklist is complete.
...
- **The context ladder is explicit.** The four supported Llama/TinyLlama rows each have a checked 512-context result; Llama 3.2 1B also has checked 1024/2048/4096/8192 packs; Llama 3.2 3B and Llama 3 8B also have checked 1024 and 2048 packs. Mixtral has no promoted context bucket; only one-token backend MoE runtime evidence is retained.
...
- **Supported-row hardening:** preserve TinyLlama as the full current gate, move the verified Llama rows toward the same normalized bar, and keep support wording tied to green evidence. The remaining gaps are broader context, arbitrary/Jinja template coverage, production-throughput evidence, portability, and repeated current-head bundles.
  - Mixtral continuation hardening is active validation only: one-token backend MoE runtime evidence exists, but later-generation parity diverges and a longer-continuation backend HTTP hang keeps API/WebUI/frontend readiness blocked.
- **Active next-model bring-up:** Mistral 7B Instruct is the lead exact-row bring-up lane; Qwen 2.5 7B Instruct and Gemma 2 9B Instruct remain planned exact-row candidates.
  - `Mistral-7B-Instruct-v0.3.Q8_0.gguf` — active validation only: source/SHA, exact tokenizer/template references, 1-token generation parity, broader five-prompt/50-token parity, checked 512/1024/2048 context bring-up, and checked 4096/8192 context evidence now exist. Support remains fail-closed pending row-specific API/WebUI/RSS evidence and synchronized capability/frontend/docs wording. No Mistral-family, neighboring-row, arbitrary-template, production-throughput, portability, or full-support claim is implied.
  - `Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf` — active validation / partial runtime only: one-token backend MoE runtime evidence is documented, but Gate 9A 50-token evidence diverges at generated token index 9 and the long-generation continuation bundle records a backend HTTP hang. No Mixtral API/WebUI/frontend readiness, neighboring-row, broad-family, long-context, arbitrary-template, production-throughput, portability, or broader/full support is claimed.
  ...
  - Public wording for the Mixtral row is limited to “one-token backend MoE runtime evidence; later-generation/API/WebUI/frontend readiness blocked.” Qwen and Gemma remain “not supported yet” until each row has its own source/SHA/license, tokenizer/template, parity, checked load/readiness, API/WebUI, RSS/timing, scrubbed manifest, and checksum evidence. See [`COMPATIBILITY.md`](COMPATIBILITY.md#locked-next-family-readiness-language) for the row-by-row promotion checklist.
...
| Exact lane | Public status | Green evidence today | Boundary |
| --- | --- | --- | --- |
| TinyLlama 1.1B Chat Q8_0 | **Verified support** | End-to-end generation, broader five-prompt/50-token parity, template-shape checks, 512-context coverage, and backend RSS/perf sampling. | This is the trusted current gate, not a promise about other TinyLlama variants or quants. |
| Llama 3.2 1B Instruct Q8_0 | **Verified end-to-end support** | Load, completions, chat completions, WebUI validation, compact/broader parity, template-shape checks, unique-chat perf/RSS sampling, and checked 512/1024/2048/4096/8192-context packs. | Supported today for this exact row at checked 512, 1024, 2048, 4096, and 8192 contexts. The 8192 pass is current-head evidence via `qa/evidence-bundles/llama32-1b-context-8192-current-head-20260513T183501Z-head-aaf9207d1669/manifest.json`; model-native/larger-context beyond those checked packs, arbitrary-template coverage, production throughput, portability, and neighboring rows are not included in this row's claim. |
| Llama 3.2 3B Instruct Q8_0 | **Verified support** | Load, completions, chat completions, WebUI validation, compact/broader 50-token parity, template-shape checks, unique-chat perf/RSS, checked 512/1024/2048-context packs, and an opt-in parallel Q8 first-token direction probe. | Supported today for this exact row at checked 512, 1024, and 2048 contexts. The parallel Q8 result is a direction probe, not production throughput; model-native/larger context, arbitrary-template, and portability evidence are not included in this row's claim. |
| Llama 3 8B Instruct Q8_0 | **Verified support** | Load, completions, chat completions, WebUI validation, compact parity, three-prompt 50-token parity, checked 512/1024/2048-context packs, compact chat-template-shapes pack, memory evidence, structured RSS/Q8 file-read counters, and lazy-Q8 hot-path measurements. | Supported today for this exact row at checked 512, 1024, and 2048 contexts. The 1024/2048 pass is exact-row/current-head only via `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T041451Z-head-8e26be0a73c0/manifest.json`; model-native/larger context beyond those checked packs, production throughput, arbitrary templates, portability, neighboring rows, and broad 8B/Llama support are not included in this row's claim. |
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | **In active validation; not supported yet** | Source/SHA, exact tokenizer/template references, 1-token generation parity, broader five-prompt/50-token parity, checked 512/1024/2048 bring-up, and checked 4096/8192 context validation are green; latest context bundle: `qa/evidence-bundles/mistral-7b-v0.3-q8-context-4096-8192-ubuntu-20260509T005229Z-head-9e3c64f2cfab/manifest.json`. | No Mistral-family support, neighboring variants, arbitrary templates, model-native/larger context, API/WebUI readiness, production throughput, portability, or full support until row-specific readiness evidence and surfaces land. |
| Mixtral-8x7B-Instruct-v0.1.Q8_0.gguf | **Active validation; partial backend runtime only** | Bounded one-token backend MoE runtime evidence exists, with tokenizer/template and sparse MoE metadata proof. Blocker anchors: `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-gate9a-50tok-20260511/`, `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-longgen-continuation-20260511/`, and `qa/evidence-bundles/mixtral-8x7b-v0.1-q8-blocker-reconciliation-20260512/`. | Later generation diverges and longer-continuation hangs; no Mixtral API/WebUI/frontend readiness, neighboring-row, long-context, arbitrary-template, production-throughput, portability, or broader/full support claim. |
| Qwen2.5-7B-Instruct-Q8_0.gguf | **Planned exact-row candidate** | Candidate row selected for acquisition/tokenizer planning only. | No Qwen support until tokenizer/template references, architecture mapping, checked load, parity, API/WebUI, RSS, and bundle evidence exist. |
| gemma-2-9b-it-Q8_0.gguf | **Planned exact-row candidate** | Candidate row selected for acquisition/tokenizer planning only. | No Gemma support until tokenizer/template references, architecture mapping, checked load, parity, API/WebUI, RSS, and bundle evidence exist. |
...
| Llama 3.2 1B Instruct Q8_0 | 8192-context recall pack | PASS on current `main` | `CMLD-819` |
...
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | Active validation: Ubuntu bring-up plus exact tokenizer/template, 1-token, broader five-prompt/50-token parity, and checked 4096/8192 context evidence | PASS for validation lane; still unsupported | API/WebUI/RSS readiness and synchronized support surfaces still required before any support claim |
...
- The next major 8B, Mistral, and Mixtral-row gaps are broader context, arbitrary templates, production throughput, portability, repeated durability evidence, and normalized support bundles; Mistral also still needs row-specific API/WebUI/RSS readiness, and Mixtral must first fix later-generation divergence and the continuation hang before any API/WebUI/frontend support claim.
