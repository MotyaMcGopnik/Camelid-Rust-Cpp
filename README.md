...
- **Four exact Q8_0 rows are public and evidence-backed.** TinyLlama remains the full current gate; Llama 3.2 1B, Llama 3.2 3B, and Llama 3 8B are supported within validated bounds.
- **Mistral and Mixtral stay fail-closed.** Mistral is an active exact-row bring-up lane; Mixtral has bounded backend MoE runtime evidence but still has a known short-prompt divergence and no broader support claim.
...
- **The context ladder is explicit.** The four supported rows have checked bounded 512-context evidence; Llama 3.2 1B/3B and Llama 3 8B also have checked 1024 and 2048 packs.
...
- **Active next-model bring-up:** Mistral 7B Instruct is the lead exact-row bring-up lane; Mixtral 8x7B Instruct is in bounded backend MoE runtime validation; Qwen 2.5 7B Instruct and Gemma 2 9B Instruct remain planned exact-row candidates.
...
**Naming note.** Camelid is the product name. The repository, crate, binary, and some diagnostics still use `backendinference` during the transition.
...
The maintainer matrix now includes four supported exact Q8_0 rows with checked row-specific evidence. Mistral remains an active validation row, and Mixtral has bounded backend MoE runtime evidence only. These are bounded checks, not universal model claims.
...
- The next major 8B, Mistral, and Mixtral support gaps are broader context, arbitrary templates, production throughput, portability, repeated durability evidence, and normalized support bundles; Mistral also still needs row-specific API/WebUI/RSS readiness, and Mixtral still has a known f16 KV-cache/KQV step-2 divergence plus missing readiness evidence.
