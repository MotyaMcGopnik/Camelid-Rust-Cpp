...
Current validation-lane blocker: Tim has intentionally shut down the Ubuntu validation host, so promotion-grade Llama-family runtime reruns and the 8B longer-context/performance lane are blocked until Tim explicitly reopens that host/runtime lane. Do not substitute local Mac llama-server/reference workloads for that paused remote validation track.
...
Current validation-lane note: promotion-grade Llama-family remote reruns are paused while Tim's Ubuntu validation host is intentionally shut down. Keep the existing exact-row smoke evidence intact, but do not run substitute local Mac llama-server/reference workloads or widen the claim until Tim reopens that runtime lane.
