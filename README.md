...
- **The context ladder is explicit.** The four supported Llama/TinyLlama rows have checked bounded 512-context evidence; Llama 3.2 1B also has a checked 4096 pack; Llama 3.2 3B and Llama 3 8B also have checked 1024 and 2048 packs. Mixtral has no promoted context bucket; only bounded one-token backend MoE runtime evidence is retained.
...
| Llama 3.2 1B Instruct Q8_0 | **Verified support (bounded)** | Load, completions, chat completions, WebUI validation, compact/broader parity, bounded template-shape checks, bounded unique-chat perf/RSS, and checked 512/1024/2048/4096-context packs. | The 4096 pass is exact-row/current-head only via `qa/evidence-bundles/llama32-1b-context-4096-current-head-20260513T155455Z-head-039353c/manifest.json`; it is not model-native/larger-context beyond checked packs, arbitrary-template, production-throughput, portability, or neighboring-row support. |
...
| Llama 3.2 1B Instruct Q8_0 | 4096-context bounded recall pack | PASS on current `main` | `CMLD-409` |
