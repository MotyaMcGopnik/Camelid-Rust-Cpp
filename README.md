...
> **Current public posture:** Camelid achieves 1:1 parity with llama.cpp for four exact GGUF rows within a bounded published validation envelope: TinyLlama at the current validated gate, Llama 3.2 1B/3B Q8_0 through checked bounded 512/1024/2048-context packs, and Llama 3 8B Q8_0 through the committed checked bounded 512-context pack. `Mistral-7B-Instruct-v0.3.Q8_0.gguf` remains an active exact-row bring-up lane, not a supported row: source/SHA, tokenizer/template, 1-token generation, bounded context, and broader five-prompt/50-token parity evidence now exist, but promotion stays fail-closed until API/WebUI/RSS readiness and the full support surfaces are synchronized. The latest committed 8B 1024/2048 PASS bundle at `qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260508T210500Z-head-160348118d44/manifest.json` is historical source-head evidence after later commits, not a current-head green claim. These are exact-row bounded-pack claims only; wider model-native context, production throughput, portability, arbitrary templates, and broad-family support remain outside the claim.
...
- **Four exact Q8_0 rows are public and evidence-backed.** TinyLlama remains the full current gate; Llama 3.2 1B and Llama 3.2 3B have checked bounded 512/1024/2048 support, while Llama 3 8B is limited to the committed checked bounded 512 pack until fresh current-head 1024/2048 PASS evidence and synchronized docs/API/frontend wording land. Mistral 7B Instruct v0.3 remains fail-closed as an active bring-up lane until its current-head promotion checklist is complete.
...
- **The context ladder is explicit.** TinyLlama stays at the current gate with bounded 512-context coverage; Llama 3.2 1B/3B have checked bounded 512/1024/2048-context evidence, and Llama 3 8B has committed checked bounded 512-context evidence. Existing 8B 1024/2048 PASS bundles are cited as historical source-head evidence only until rerun on the current head and aligned across docs/API/frontend. Mistral context evidence remains bring-up evidence only until the exact row is promoted through the support checklist. These checked packs do not imply model-native/larger context, arbitrary-template, production-throughput, portability, or broader-family support.
...
| Llama 3 8B Instruct Q8_0 | **Verified support (bounded)** | Load, completions, chat completions, WebUI validation, compact parity, three-prompt 50-token parity, the checked 512-context pack, compact chat-template-shapes pack, bounded memory evidence, structured RSS/Q8 file-read counters, and lazy-Q8 hot-path measurements. | 1024/2048 PASS bundles exist only for earlier source/runtime heads (latest committed: `160348118d44`); current `main` needs a fresh canonical PASS plus docs/API/frontend alignment before those buckets are promoted. No model-native/larger context, production throughput, arbitrary templates, portability, neighboring-row, or broad 8B/Llama support is implied. |
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | **In active validation; not supported yet** | Source/SHA, exact tokenizer/template references, 1-token generation parity, bounded 512/1024/2048 bring-up, and broader five-prompt/50-token parity are green; latest broader bundle: `qa/evidence-bundles/mistral-7b-v0.3-q8-broader-50tok-ubuntu-20260509T000633Z-head-d330e97ae992/manifest.json`. | No Mistral-family support, neighboring variants, arbitrary templates, model-native/larger context, API/WebUI readiness, production throughput, portability, or full support until row-specific readiness evidence and surfaces land. |
...
| Mistral-7B-Instruct-v0.3.Q8_0.gguf | Active validation: Ubuntu bounded bring-up plus exact tokenizer/template, 1-token, and broader five-prompt/50-token parity evidence | PASS for validation lane; still unsupported | API/WebUI/RSS readiness and synchronized support surfaces still required before any support claim |
...
- “Llama support” currently means only the exact supported rows above; Mistral has no support claim yet and may only be discussed as the exact active validation row above.
...
- The next major 8B and Mistral support gaps are fresh current-head 8B 1024/2048 PASS evidence with docs/API/frontend alignment; for Mistral, row-specific API/WebUI/RSS readiness and synchronized capability/frontend/docs surfaces; plus model-native/larger context beyond checked packs, arbitrary templates, production throughput, portability, repeated durability evidence, and normalized full-support bundles.
