...
assert.match(chatWorkspaceSource, /data-live-phase=\{phase \|\| 'streaming'\}/, 'visible still-generating status badges should expose the current streaming phase for regressions')
assert.match(chatWorkspaceSource, /ACTIVE_STREAMING_LABEL\s*=\s*'Still generating — streaming tokens as Camelid generates them'/, 'live assistant rows should keep an explicit still-generating status while the backend is active')
assert.match(chatWorkspaceSource, /OPEN_CODE_STREAMING_LABEL\s*=\s*'Still generating — streaming code block is still open'/, 'streaming open code fences should visibly say the code block is still incomplete')
...
assert.match(chatWorkspaceSource, /PREPARING_STREAMING_LABEL\s*=\s*'Still generating — preparing local request and connecting to Camelid'/, 'pre-token pending rows should visibly say the local stream is being prepared')
assert.match(chatWorkspaceSource, /FIRST_TOKEN_STREAMING_LABEL\s*=\s*'Still generating — connected and waiting for the first token'/, 'pre-token pending rows should visibly say the backend is still generating')
assert.match(chatWorkspaceSource, /LONG_FIRST_TOKEN_STREAMING_LABEL\s*=\s*'Still generating — local prefill is taking a while'/, 'long pre-token waits should keep a visible still-generating explanation')
