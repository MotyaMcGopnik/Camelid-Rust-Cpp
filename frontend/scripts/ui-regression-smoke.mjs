...
assert.match(chatWorkspaceSource, /function StreamingStatus[\s\S]*Still generating locally/, 'live assistant rows should keep an explicit still-generating status while the backend is active')
assert.match(chatWorkspaceSource, /message\.role === 'assistant' && message\.streaming && <StreamingStatus/, 'token-streaming assistant rows should render the active status badge')
