...
assert.match(apiViewSource, /exactRowSupportLanes\(selectedCompatibilityTarget, apiFeatures\)/, 'API support view should show template/Jinja and throughput readiness lanes for the selected exact row')
...
assert.match(modelsViewSource, /template\/Jinja and production-throughput shown as row-scoped readiness lanes instead of repeated generic caveats/, 'Models support section should state that template/Jinja and production-throughput are row-scoped lanes, not generic caveats')
