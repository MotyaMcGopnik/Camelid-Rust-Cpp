...
assert.match(chatWorkspaceSource, /message\.role === 'assistant' && message\.streaming && \(\s*<div className="message-live-status"/s, 'live assistant rows should keep an explicit active status while the backend is still generating')
...
assert.match(componentCss, /assistant:not\(\.is-streaming\)::after\s*{[^}]*content:\s*none/s, 'completed assistant rows should explicitly suppress pellet pseudo-content')
