...
assert.match(chatWorkspaceSource, /function PacmanLoader/, 'pre-token live status should render through the explicit Pac-Man loader component')
assert.match(chatWorkspaceSource, /role="status" aria-live="polite"/, 'Pac-Man loader should expose an accessible live status while the backend is active')
assert.match(chatWorkspaceSource, /aria-label=\{`\$\{label\}\. \$\{elapsedSeconds\} seconds elapsed\.`\}/, 'Pac-Man loader should keep the status label and elapsed time available to assistive tech')
assert.match(chatWorkspaceSource, /pacman-loader-mouth/, 'Pac-Man loader should render the chomping mouth element explicitly')
assert.match(chatWorkspaceSource, /pacman-loader-pellet pacman-loader-pellet-5/, 'Pac-Man loader should render the pellet trail explicitly')
...
assert.match(chatWorkspaceSource, /showStreamingStatus && <PacmanLoader/, 'pre-token assistant rows should render the active Pac-Man loader before streamed content')
...
assert.doesNotMatch(chatWorkspaceSource, /<StreamingStatus/, 'the old text badge component should not reappear beside the dedicated Pac-Man loader')
assert.doesNotMatch(chatWorkspaceSource, /The first token has not arrived yet\./, 'pre-token assistant rows should not duplicate the Pac-Man loader with placeholder copy')
...
assert.match(dashboardHookSource, /begin immediately with the runnable code/, 'code-first prompt should suppress slow prose preambles before code')
assert.match(dashboardHookSource, /start exactly with ```html followed by <!doctype html>/, 'HTML code prompts should request visible code at the beginning of the stream')
...
assert.match(componentCss, /\.pacman-loader\s*{[^}]*display:\s*flex[^}]*justify-content:\s*center/s, 'streaming assistant rows should show a centered dedicated Pac-Man loader')
assert.match(componentCss, /\.pacman-loader-compact\s*{[^}]*padding:\s*0 0 8px/s, 'compact Pac-Man loader should sit above pre-token assistant content without extra copy')
assert.match(componentCss, /\.pacman-loader-track\s*{[^}]*width:\s*340px[^}]*height:\s*64px[^}]*overflow:\s*hidden/s, 'Pac-Man loader should keep a stable clipped game track')
assert.match(componentCss, /\.pacman-loader-mouth\s*{[^}]*z-index:\s*2[^}]*animation:\s*camelid-pacman-chomp/s, 'Pac-Man loader mouth should chomp above pellets while the loader is rendered')
assert.match(componentCss, /\.pacman-loader-pellet\s*{[^}]*border-radius:\s*999px[^}]*animation:\s*camelid-pellet-slide/s, 'Pac-Man loader pellets should be round dots that slide while the loader is rendered')
...
const pacmanRule = componentCss.match(/\.pacman-loader-mouth\s*{[\s\S]*?\n}/)?.[0] || ''
assert.match(pacmanRule, /width:\s*60px/, 'Pac-Man loader mouth should keep its large loader size')
assert.match(pacmanRule, /height:\s*60px/, 'Pac-Man loader mouth should keep its large loader size')
assert.match(pacmanRule, /position:\s*absolute/, 'Pac-Man loader mouth should stay anchored inside the track')
assert.match(pacmanRule, /clip-path:\s*polygon/, 'Pac-Man should use a wedge mouth')
const streamingPelletRule = componentCss.match(/\.pacman-loader-pellet\s*{[\s\S]*?\n}/)?.[0] || ''
assert.match(streamingPelletRule, /width:\s*10px/, 'streaming pellets should stay small and game-like')
assert.match(streamingPelletRule, /height:\s*10px/, 'streaming pellets should stay small and game-like')
assert.match(streamingPelletRule, /right:\s*-16px/, 'pellets should feed in from the right side of the track')
const pelletKeyframes = componentCss.match(/@keyframes camelid-pellet-slide\s*{[\s\S]*?\n}/)?.[0] || ''
assert.match(pelletKeyframes, /var\(--pacman-pellet-travel\)/, 'pellet animation should travel into Pac-Man instead of drifting in place')
assert.match(pelletKeyframes, /opacity:\s*0/, 'pellets should disappear as Pac-Man eats them')
assert.doesNotMatch(pelletKeyframes, /translateY/, 'pellet animation should stay game-steady without bobbing')
...
assert.doesNotMatch(pacmanAndPellets, /translateY|translate3d/, 'Pac-Man and pellet styling should not bob or float')
assert.doesNotMatch(componentCss, /\.message-row-gemini\.assistant\.is-streaming::after\s*{/, 'streaming assistant rows must not keep legacy pseudo pellet animation')
