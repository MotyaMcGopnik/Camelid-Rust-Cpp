...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_smoke_only', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: '8B 1024/2048 remain red until fresh PASS artifacts plus docs/API/frontend alignment exist; model-native/larger context beyond the checked 512 pack, arbitrary templates, throughput, portability, repeated current-head evidence, and durable normalized full-support bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_512_pack: 'validated_first_pack', bounded_context_1024_pack: 'not_promoted', bounded_context_2048_pack: 'not_promoted', latest_checked_bucket: 'llama3-context-512-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'CMLD-512-CHECK', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, checked 512-context pack, compact template-shapes pack evidence, and bounded memory/hot-path measurements. Historical or unaligned 8B 1024/2048 artifacts are not support promotion; no model-native/larger context or broader/full support is implied.' },
...
assert.match(
  ...
  'ModelsView should keep 8B 1024/2048 warm until fresh current-head evidence and docs/API/frontend alignment exist',
)
assert.doesNotMatch(
  modelsViewSource,
  /pin-badge ready[^>]*>8B 1024\/2048 bounded packs passed</,
  'ModelsView must not show stale green 8B 1024/2048 copy before fresh current-head PASS and alignment land',
)
...
assert.match(compatibilityHintCopy(llama3EightBHint), /checked 512-context pack, compact template-shapes pack evidence, and bounded memory\/hot-path measurements/)
assert.match(compatibilityHintCopy(llama3EightBHint), /Historical or unaligned 8B 1024\/2048 artifacts are not support promotion/)
