...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_smoke_only', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: 'model-native/larger context beyond the checked 512/1024/2048 bounded packs, arbitrary templates, throughput, portability, and durable normalized full-support bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_512_pack: 'validated_first_pack', bounded_context_1024_pack: 'validated_second_pack', bounded_context_2048_pack: 'validated_third_pack', latest_checked_bucket: 'llama3-context-2048-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'CMLD-204', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, checked 512/1024/2048-context packs, compact template-shapes pack evidence, and current-head 1024/2048 artifacts with RSS/q8_file_reads counters. Camelid supports exact-row smoke plus the checked bounded 512/1024/2048 packs only.' },
...
assert.match(
  ...
  'ModelsView must show green 8B 1024/2048 bounded-pack copy only after current-head evidence and docs/API/frontend alignment exist',
)
assert.doesNotMatch(
  modelsViewSource,
  /8B 1024\/2048 remain red pending fresh current-head PASS/,
  'ModelsView must not keep 8B 1024/2048 red after current-head PASS artifacts and alignment',
)
...
assert.match(compatibilityHintCopy(llama3EightBHint), /checked 512\/1024\/2048-context packs, compact template-shapes pack evidence, and current-head 1024\/2048 artifacts with RSS\/q8_file_reads counters/)
assert.doesNotMatch(compatibilityHintCopy(llama3EightBHint), /not promoted on this head/)
