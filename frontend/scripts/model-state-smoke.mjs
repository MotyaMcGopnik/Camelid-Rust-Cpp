...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_smoke_only', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: 'model-native/larger context beyond checked packs, arbitrary templates, throughput, portability, and durable bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_512_pack: 'validated_first_pack', bounded_context_1024_pack: 'blocked_pending_current_head_pass_and_alignment', bounded_context_2048_pack: 'blocked_pending_current_head_pass_and_alignment', latest_checked_bucket: 'llama3-context-512-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'CMLD-512-CHECK', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, checked 512-context pack, and compact template-shapes pack evidence; 8B 1024/2048 remain red until fresh current-head PASS artifacts and synchronized docs/API/frontend copy exist' },
    { id: 'mistral_7b_instruct_v0_3_q8_0', family: 'mistral', quantization: 'Q8_0', status: 'acceptance_target', support_scope: 'bringup_exact_row_unsupported', full_support_status: 'blocked_unsupported_bringup', full_support_blockers: 'bounded load/readiness, API/WebUI, RSS/timing, checksums, and durable row-specific bundle evidence are missing', evidence: 'Mistral v0.3 tokenizer/template and 1-token/5-token smoke evidence only' },
...
assert.match(
  ...
  'ModelsView must keep 8B 1024/2048 red until fresh current-head PASS artifacts and docs/API/frontend alignment exist',
)
assert.doesNotMatch(
  modelsViewSource,
  /8B 1024\/2048 bounded packs passed/,
  'ModelsView must not show stale 8B 1024/2048 pass copy after API capabilities mark those exact bounded packs red',
)
...
assert.deepEqual(
  ...
  [
    ...
    ['llama3_8b_instruct_q8_0', 'blocked_pending_current_head_pass_and_alignment'],
  ],
  'frontend tracked rows should preserve the API 1024-context boundary: TinyLlama not promoted, exact 1B/3B promoted, and exact 8B red pending current-head proof plus alignment',
)
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 2048-context boundary: TinyLlama not promoted, exact 1B/3B promoted, and exact 8B red pending current-head proof plus alignment',
)
assert.deepEqual(
  ...
  [
    ...
    ['llama3_8b_instruct_q8_0', 'llama3-context-512-smoke-v1', 'pass', 'CMLD-512-CHECK'],
  ],
...
assert.match(compatibilityHintCopy(llama3EightBHint), /checked 512-context pack, and compact template-shapes pack evidence; 8B 1024\/2048 remain red until fresh current-head PASS artifacts and synchronized docs\/API\/frontend copy exist/)
