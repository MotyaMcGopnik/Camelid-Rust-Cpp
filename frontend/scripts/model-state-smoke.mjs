...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_smoke_only', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: 'model-native/larger context, arbitrary templates, throughput, portability, and durable bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_1024_pack: 'not_promoted', bounded_context_2048_pack: 'not_promoted', latest_checked_bucket: 'llama3-context-512-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'context-512-pack-pass', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, first 512-context, and compact template-shapes pack evidence; 1024/2048 remain not promoted' },
...
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 1024-context boundary: TinyLlama not promoted and 8B still not promoted',
)
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 2048-context boundary: TinyLlama not promoted and 8B still not promoted',
)
assert.deepEqual(
  ...
  [
    ...
    ['llama3_8b_instruct_q8_0', 'llama3-context-512-smoke-v1', 'pass', 'context-512-pack-pass'],
  ],
  'frontend tracked rows should surface the API latest bounded checks without implying broad/full support or promoting 8B 1024/2048',
)
