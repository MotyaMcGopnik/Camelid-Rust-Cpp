...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_smoke_only', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: 'model-native/larger context beyond the checked 512 bounded pack, 1024/2048 current-head PASS artifacts plus docs/API/frontend alignment, arbitrary templates, throughput, portability, and durable normalized full-support bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_512_pack: 'validated_first_pack', bounded_context_1024_pack: 'not_promoted', bounded_context_2048_pack: 'not_promoted', latest_checked_bucket: 'llama3-context-512-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'not_published', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, checked 512-context pack, compact template-shapes pack evidence, and older source-head 1024/2048 artifacts that are not promoted on this head. Camelid supports exact-row smoke plus the checked bounded 512 pack only. 8B 1024/2048 remain red pending fresh current-head PASS artifacts plus docs/API/frontend alignment.' },
...
assert.match(
  modelsViewSource,
  /pin-badge warm[^>]*>8B 1024\/2048 remain red pending fresh current-head PASS</,
  'ModelsView must keep 8B 1024/2048 visibly non-green until fresh current-head PASS artifacts plus docs/API/frontend alignment exist',
)
assert.doesNotMatch(
  modelsViewSource,
  /8B 1024\/2048 bounded packs passed for exact row/,
  'ModelsView must not show green 8B 1024/2048 bounded-pack copy without current-head evidence and alignment',
)
...
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 1024-context boundary: TinyLlama and exact 8B not promoted, exact 1B/3B promoted only for their checked bounded packs',
)
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 2048-context boundary: TinyLlama and exact 8B not promoted, exact 1B/3B promoted only for their checked bounded packs',
)
assert.deepEqual(
  ...
  [
    ...
    ['llama3_8b_instruct_q8_0', 'llama3-context-512-smoke-v1', 'pass', 'not_published'],
  ],
  'frontend tracked rows should surface the API latest bounded checks without promoting 8B 1024/2048 or implying broad/full support or model-native/larger-context support',
)
...
assert.match(compatibilityHintCopy(llama3EightBHint), /checked 512-context pack, compact template-shapes pack evidence, and older source-head 1024\/2048 artifacts that are not promoted on this head/)
assert.doesNotMatch(compatibilityHintCopy(llama3EightBHint), /checked 512\/1024\/2048 context packs/)
