...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', bounded_context_1024_pack: 'validated_second_pack', bounded_context_2048_pack: 'validated_third_pack', latest_checked_bucket: 'llama3-context-2048-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'CMLD-204', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, first 512-context, current-head 1024/2048-context, and compact template-shapes pack evidence' },
  ],
}
...
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 1024-context pack boundary: 1B/3B/8B passed bounded second packs',
)
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 2048-context boundary: 1B/3B/8B passed bounded third packs',
)
assert.deepEqual(
...
assert.match(compatibilityHintCopy(llama3EightBHint), /first 512-context, current-head 1024\/2048-context, and compact template-shapes pack evidence/)
