...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_bounded_pack_support', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: 'model-native/larger context beyond the checked 512 pack, fresh 1024/2048 current-head reruns, arbitrary templates, throughput, portability, and durable normalized full-support bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_512_pack: 'validated_first_pack', bounded_context_1024_pack: 'not_promoted_current_head_rerun_required', bounded_context_2048_pack: 'not_promoted_current_head_rerun_required', latest_checked_bucket: 'llama3-context-512-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'CMLD-512', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, checked 512 context pack, compact template-shapes pack evidence, and historical 1024/2048 source-head PASS artifacts that require current-head rerun before promotion. Camelid supports exact-row smoke plus those checked bounded packs only. No model-native/larger context, broad/full support, neighboring row, arbitrary template, throughput, or portability claim is implied' },
    { id: 'mistral_7b_instruct_v0_3_q8_0', family: 'mistral', quantization: 'Q8_0', status: 'active_validation_unsupported', support_scope: 'bringup_exact_row_unsupported', full_support_status: 'blocked_unsupported_bringup', full_support_blockers: 'source/SHA/license, tokenizer/template parity, prompt-token references, bounded load/readiness, API/WebUI, RSS/timing, scrubbed manifest, checksums, and durable row-specific bundle evidence are not complete as promotion evidence', evidence: 'Mistral v0.3 active validation only; no support evidence exists' },
...
assert.match(
  modelsViewSource,
  /pin-badge warm[^>]*>8B 1024\/2048 remain red pending fresh current-head PASS \+ docs\/API\/frontend alignment</,
  'ModelsView must show the red 8B 1024/2048 boundary until fresh current-head PASS artifacts plus docs/API/frontend alignment exist',
)
assert.doesNotMatch(
  modelsViewSource,
  /8B 1024\/2048 bounded packs passed at current head/,
  'ModelsView must not show stale green 8B 1024/2048 current-head copy until API capabilities and evidence promote the checked bounded packs',
)
...
assert.deepEqual(
  ...
  [
    ...
    ['llama3_8b_instruct_q8_0', 'not_promoted_current_head_rerun_required'],
  ],
  'frontend tracked rows should preserve the API 1024-context boundary: TinyLlama not promoted, exact 1B/3B promoted only for their checked bounded packs and 8B pending current-head rerun',
)
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 2048-context boundary: TinyLlama not promoted, exact 1B/3B promoted only for their checked bounded packs and 8B pending current-head rerun',
)
assert.deepEqual(
  ...
  [
    ...
    ['llama3_8b_instruct_q8_0', 'llama3-context-512-smoke-v1', 'pass', 'CMLD-512'],
  ],
...
assert.match(compatibilityHintCopy(llama3EightBHint), /checked 512 context pack, compact template-shapes pack evidence, and historical 1024\/2048 source-head PASS artifacts that require current-head rerun before promotion/)
...
assert.match(mistralExactHint.target.full_support_blockers, /API\/WebUI|durable row-specific bundle/i, 'Mistral exact-row matching must carry its blocking evidence list')
...
assert.equal(mixtralHint.kind, 'compatibility', 'Mixtral should match only its exact future planning row, not a Mistral exact-row match')
...
assert.equal(mixtralNoQuantHint.kind, 'quant_missing', 'Mixtral exact planning rows must still require quant evidence')
