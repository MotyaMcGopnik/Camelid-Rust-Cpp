...
const capabilityFixture = {
  ...
  model_compatibility: [
    ...
    { id: 'llama3_8b_instruct_q8_0', family: 'llama_bpe_decoder', quantization: 'Q8_0', status: 'supported_exact_row_smoke', support_scope: 'exact_row_smoke_only', full_support_status: 'blocked_pending_normalized_full_support', full_support_blockers: 'model-native/larger context beyond checked packs, arbitrary templates, throughput, portability, and durable bundles remain missing', frontend_readiness_gate: 'green only when this exact GGUF row plus Q8_0 quant match /api/capabilities and the runtime reports loaded_now=true, generation_ready=true, and matching active_model_id', bounded_context_1024_pack: 'validated_second_pack', bounded_context_2048_pack: 'validated_third_pack', latest_checked_bucket: 'llama3-context-2048-smoke-v1', latest_checked_result: 'pass', latest_checked_output: 'CMLD-204', evidence: '8B exact-row API/frontend smoke plus compact 50-token, broader 50-token, checked 512/1024/2048-context packs, and compact template-shapes pack evidence; model-native/larger context beyond checked packs remains unpromoted' },
...
assert.match(
  ...
  'ModelsView must show the exact 8B 1024/2048 bounded-pack pass only after docs/API/frontend are aligned',
)
assert.doesNotMatch(
  modelsViewSource,
  /8B 1024\/2048 not promoted/,
  'ModelsView must not keep stale 8B 1024/2048 blocked copy after API capabilities mark those exact bounded packs promoted',
)
...
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 1024-context boundary: TinyLlama not promoted and exact 1B/3B/8B rows promoted only for checked packs',
)
assert.deepEqual(
  ...
  'frontend tracked rows should preserve the API 2048-context boundary: TinyLlama not promoted and exact 1B/3B/8B rows promoted only for checked packs',
)
assert.deepEqual(
...
assert.match(compatibilityHintCopy(llama3EightBHint), /checked 512\/1024\/2048-context packs, and compact template-shapes pack evidence; model-native\/larger context beyond checked packs remains unpromoted/)
