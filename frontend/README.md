...
All three rows still fail closed in the WebUI unless the active local GGUF matches the exact row and `/v1/health` reports `loaded_now=true` plus `generation_ready=true`. Arbitrary/Jinja-template readiness and production-throughput readiness now appear as supported exact-row lanes when `/api/capabilities` reports green template-shape/renderer and perf/RSS evidence; the cards still do not infer neighboring sizes, base variants, other quantizations, larger contexts beyond checked packs, or portability.
