...
- **Llama 3.2 1B Instruct Q8_0:** exact-row API/WebUI smoke plus compact/broader parity and bounded 512/1024/2048 context-pack evidence are represented as a supported exact-row smoke lane; the 2048 pack is exact-row only after the RoPE frequency-factor fix.
- **Llama 3.2 3B Instruct Q8_0:** exact-row API/WebUI smoke, compact parity, broader three-prompt 50-token parity, five-prompt API smoke, and bounded 512/1024/2048 context packs are represented as a supported exact-row smoke lane.
- **Llama 3 8B Instruct Q8_0:** exact-row API/WebUI smoke, clean-main timing/RSS smoke, broader 50-token parity, the bounded 512-context pack, compact chat-template-shapes pack, structured RSS/Q8 file-read counters, and measurement-only lazy-Q8 hot-path cost probe are represented as bounded exact-row wins only. The newest committed 1024/2048 bounded-pack PASS at `../qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T024342Z-head-b49034007f2e/manifest.json` is historical after later runtime/source/support-surface changes, so frontend copy must keep 1024/2048 warm/red until a fresh current-head PASS and docs/API/frontend alignment land. None of this is model-native/larger-context, broad 8B/Llama, production-throughput, or portability support.
...
These commands must still fail closed if the loaded model is the wrong row, lacks Q8_0 metadata, is not `loaded_now=true` + `generation_ready=true`, or is outside the exact supported `/api/capabilities` row. That is intentional: the UI supports only the exact 1B/3B/8B smoke rows without making a broad Llama-family claim. The latest public reopened-lane API + frontend smoke summary for all four exact rows is `../qa/evidence-bundles/four-row-api-webui-20260505T003100Z-head-b403884/manifest.json`; use it as frontend freshness evidence, not as broader/full-support evidence. The exact 1B third bounded 2048-context pack now passed after the RoPE frequency-factor fix at `../qa/evidence-bundles/llama32-1b-context-2048-rope-factors-20260506T0105Z-head-62f8cbc/manifest.json`; the older 1B blocker notes remain historical. The exact 3B third bounded 2048-context pack passed at `../qa/evidence-bundles/llama32-3b-context-2048-20260505T105742Z-head-36ec8e492d65/manifest.json`, and the exact 8B broader three-prompt 50-token pack passed at `../qa/evidence-bundles/llama3-8b-broader-50tok-20260505T005031Z-head-d13541ad8d7e/manifest.json`, the first bounded 512-context pack passed at `../qa/evidence-bundles/llama3-8b-context-512-20260504T234625Z-head-58acf592345c/manifest.json`, the newest committed 8B 1024/2048 bounded-pack pass at `../qa/evidence-bundles/llama3-8b-context-1024-2048-current-head-20260509T024342Z-head-b49034007f2e/manifest.json` matched prompt tokens, generated token IDs, and generated text for `CMLD-102`/`CMLD-204` but remains historical after later runtime/source/support-surface changes, older 8B 1024/2048 bounded-pack artifacts remain historical for their source heads only, and frontend must keep 8B 1024/2048 unpromoted until fresh current-head evidence plus aligned API/docs exists, the compact chat-template-shapes pack passed at `../qa/evidence-bundles/llama3-8b-chat-template-shapes-20260505T003821Z-head-d13541ad8d7e/manifest.json`, and the lazy-Q8 hot-path measurement is summarized at `../qa/evidence-bundles/llama3-8b-lazy-q8-hotpath-20260505T021411Z-head-723a665/manifest.json`; these are bounded packs/measurements only and do not promote model-native/larger context, arbitrary-template behavior, production throughput, portability, neighboring rows, or broad Llama-family support.
