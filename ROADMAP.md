...
Executive summary: Camelid now has the TinyLlama supported gate plus exact Llama 3.2 1B/3B Q8_0 rows through bounded 2048-context packs on current `main`; Llama 3 8B Q8_0 is promoted only through its bounded 512-context pack. The 8B 1024/2048 PASS bundle at `e203d3cf3ea5` is historical after later backend changes; 8B 1024/2048 remain red pending fresh current-head PASS artifacts plus docs/API/frontend alignment. Broader/full 8B support remains gated on model-native/larger context, arbitrary templates, production throughput, portability, and repeated durability evidence.
...
- **Supported generation gates:** TinyLlama 1.1B Chat Q8_0 remains supported; exact Llama 3.2 1B/3B rows are supported through checked bounded 512/1024/2048-context packs, while the exact Llama 3 8B row is supported through the checked bounded 512-context pack on current `main`.
...
- **8B promoted lane:** Llama 3 8B Instruct Q8_0 now has compact parity, a three-prompt 50-token Ubuntu parity run, API/frontend smoke, bounded memory evidence, a checked bounded 512-context pack, and one bounded compact chat-template-shapes pack for the exact tracked Q8_0 GGUF; 1024/2048 remain red pending fresh current-head PASS artifacts plus docs/API/frontend alignment, and broader/full 8B support remains gated.
...
Near-term thesis: protect the trusted TinyLlama gate plus the exact Llama 3.2 1B/3B bounded-2048 rows and the exact Llama 3 8B bounded-512 row; broaden only with stronger row-specific evidence while every public surface stays synchronized with the exact support boundary.
...
- Llama 3 8B Q8_0 moved from groundwork-only to supported exact-row smoke after Ubuntu three-prompt parity, API/frontend smoke, bounded memory evidence, the checked bounded 512-context pack, and compact chat-template-shapes packs aligned for that exact row only; 1024/2048 remain red without fresh current-head PASS artifacts plus docs/API/frontend alignment.

Near-term objective: preserve the supported TinyLlama gate, exact Llama 3.2 1B/3B bounded-2048 lanes, and exact Llama 3 8B bounded-512 lane; do not widen to 8B 1024/2048, model-native/larger context, arbitrary templates, production throughput, portability, or adjacent rows without new evidence.
...
- Protect the exact Llama 3.2 1B/3B bounded-2048 rows and exact Llama 3 8B bounded-512 row.
...
- Preserve the Llama 3 8B exact-row promotion through the checked 512-context pack on current `main`; 1024/2048 remain red pending fresh current-head PASS artifacts plus docs/API/frontend alignment, and older 1024/2048 bundles remain historical source-head evidence only.
...
| Llama 3 8B Instruct Q8_0 exact-row smoke | Complete / narrow support through checked 512 bounded pack | Compact prompt-token/1-token/5-token/50-token parity, the three-prompt 50-token pack, API smoke, frontend smoke, bounded memory evidence, the checked bounded 512-context pack, and the compact chat-template-shapes pack support this exact 8B Q8_0 row only; 1024/2048 remain red pending fresh current-head PASS artifacts plus docs/API/frontend alignment. |
...
- Llama 3 8B Q8_0 is supported as an exact-row smoke/parity lane with compact parity, the three-prompt 50-token pass, API/frontend smoke, bounded memory evidence, the checked bounded 512-context pack, and one compact chat-template-shapes pack; 1024/2048 remain red pending fresh current-head PASS artifacts plus docs/API/frontend alignment, and model-native/larger context beyond checked packs, broader chat-template, production performance, and portability expansion remain gated.
